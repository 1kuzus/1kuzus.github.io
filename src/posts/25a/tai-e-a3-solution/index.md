NJU程序分析[死代码检测的Lab](https://tai-e.pascal-lab.net/pa3.html)关注以下两种类型的死代码：

1. 不可达代码：控制流不可达（比如`return`之后的语句）和分支不可达（比如`if(false)`）
1. 无用赋值：赋值语句的左值不在活跃变量集合中（并且等号右边的表达式没有副作用）
