...
**预期**：页面顶部依次为文章大标题（28px，来自 `archives.json` 而非正文）、一行元信息（等宽字体、灰色小字）。计数器按需拼接：`likes` 只在点赞数 > 0 时出现，`views` 只在浏览数 > 0 时出现，日期始终在末尾，因此常见形态是 `N likes · N views · 2026-08-13`，新文章未点赞时可能只有 `N views · 2026-08-13` 或仅日期。页面右上/侧边有"本页目录"面板，页面最底部有一个心形点赞按钮。目录里应当只出现 `##` 和 `###` 两级标题（即本页的"一、二、三…"和"1.1、1.2…"），**不应**出现 `####` 级别的条目。**异常**：元信息出现字面量 `null likes` / `null views`（说明把 `null` 直接拼进了字符串）；目录为空或目录里混入了 `####` 标题；滚动时目录高亮项与当前实际阅读位置错位超过一屏。
...
普通段落包含**加粗文本**、`行内代码`、[超链接](https://example.com)、_斜体_、~~删除线~~、以及自动识别的裸链接 https://example.com 和 www.example.com 。
...
**预期**：第一条公式**左对齐**（`@xprops alignLeft` 从 markdown 传到 `X.Formula`）；第二条没有 xprops，仍**水平居中**，与 4.3 一致。两条公式内容相同，便于并排对照。**异常**：两条都居中（说明 `pre` 上的 `alignLeft` 仍被丢掉）；两条都左对齐（说明默认居中坏了）。
...
**预期**：块内表格的单元格底色（灰）与引用块底色（绿）叠加后仍能看出层次；分割线横跨引用块内容区宽度；图片水平居中；嵌套引用块表现为绿底上叠一层金底、金底上再叠一层蓝底。**异常**：嵌套引用块没有内缩（说明 padding 没生效，看起来像同级）；块内分割线溢出到引用块外面。
...
### 11.7 紧凑项内行内元素与子列表混排（渲染层包装规整）

- 含`行内代码`与公式 $x^2$ 的紧凑父项
    - 子项一
    - 子项二
- 纯文本紧凑父项
    - 子项
- **加粗**开头、以[链接](https://example.com)结尾的紧凑父项
    - 子项

**预期**：紧凑父项的行内内容（无论是否含行内元素、行内元素在何位置）都会在渲染层被 `normalizeLiChildren` 统一包装为 `<p>`，因此"父项文字 → 子列表"的间距**恒为 12px**，与行内元素的有无和种类无关；每个"带子列表的父项 → 下一个父项"的间距同样恒为 12px。**异常**：某个父项的文字与子列表贴在一起（0px，说明该项的行内片段没有被包装、子列表数不到前置元素兄弟）；或者间距随内容不同时而 12px 时而 24px。
...
**预期**：第一个父项没有任何文字，渲染为"只有圆点标记的一行 + 子列表"，**不应**出现空段落占位（包装逻辑会丢弃纯空白片段，不产生空 `<p>`）；第二项内部有一个 `<br>` 硬换行、整体仍是同一段（`<br>` 是行内元素，会与前后文本一起被包进同一个 `<p>`）；第三项两行合并为一行。**异常**：空父项内部出现多余的 12px 空白（说明产生了空 `<p>`）；硬换行丢失或被拆成两段。
...
**预期**：li 内的 `####` 正常渲染为 X.H3 样式（18px、无前缀符号），与上下内容间距 12px（列表上下文收紧）——本用例特意用 `####` 而不是 `##`/`###`，因为后两者写在 li 内**会混进右侧目录**（TOC 按 class 全页收集，这是既定行为）；第二项中 ul 与 ol 两个子列表之间间距 12px，且它们之间**不产生空段落**（连续块级元素之间的空白文本片段被丢弃）。**异常**：li 内标题字号错乱或带出前缀符号；ul 与 ol 之间出现 24px 间距、粘连或空 `<p>`。
...
**预期**：三列分别左/中/右对齐，**表头单元格也跟随该列的对齐方式**。**异常**：只有数据行对齐生效而表头仍然居中；或对齐完全不生效（三列都居中，说明 GFM 的 align 没有经原生管线 `mdast-util-to-hast` → `hast-util-to-estree` 转成单元格的 `text-align` 内联样式）。
...
**预期**：第一个表三列宽度依次**精确为** 250/120/80 px；第二个表首列宽度由内容决定（`0` 被当作"不设置"，实测约 89px）、次列 300px；第三个表说明 `width` 支持带方括号的写法，两列各 200px。用 DevTools 选中任一单元格看 Computed 宽度即可核对。三个表**都保留深色表头行** —— 传了 `width` 而不传 `thead` 时，表格走"重建"分支把行拍进单个 `<tbody>`，但会回退到"原本是 `<th>` 的格子仍然是表头"的判定，所以视觉上与默认表格一致。**异常**：深色表头消失（说明回退判定被改坏了）；方括号写法把 `[200` 解析成 NaN 导致列宽塌陷；某一列的宽度只作用于表头行而没有作用于数据行。
...
### 13.5 单元格内多个块级元素（单元格 owl 间距）

| 双图单元格                    | 文字加图            |
| ----------------------------- | ------------------- |
| ![](small.gif) ![](small.gif) | 文字 ![](small.gif) |

**预期**：图片是 `display: block`，同一单元格内的两张图会竖排；左格两图之间有 **8px** 间距（`flow.css` 的 `td > * + *` 规则，这是 markdown 里唯一能触发它的场景）；右格中"文字"是裸文本节点、图片数不到前置元素兄弟，文字与图片之间**没有**注入间距（0px，既定行为——单元格内不做渲染层包装，GFM 单元格本就只支持行内内容，此处影响可忽略）。**异常**：左格两图间距为 0（说明单元格 owl 规则失效）或 24px（说明容器规则错误命中）；右格图片上方凭空多出间距。
...
**预期**：四张图都**水平居中**、与上下内容间距 24px、宽度不超过正文列（`max-width: 100%`）、高度按原比例自动缩放（不变形）。GIF 应当在播放动画。第三张图没有写 alt，其 `alt` 属性会被兜底为字符串 `image`。图片区域不可被文本选中（`user-select: none`），鼠标悬停为放大镜光标（`zoom-in`），点击进入灯箱，详见 14.6。**异常**：图片被拉伸变形（说明 `height: auto` 失效）；加载过程中页面发生跳动（CLS，说明宽高属性没生效）；超宽图片撑破正文列。
...
本站所有 `.x-image`（含列表/引用块/表格内的图，以及 14.4 里被拆成独占一行的"行内"图）点击后应打开全屏灯箱。

**预期**：
- 点击任意图片，屏幕中央出现原图放大预览，背后是半透明黑遮罩（`#000000c0`），鼠标为 `zoom-out`；
- 图片被约束在约 `96vw × 92vh` 内，不撑出视口，比例不失真；
- 点击遮罩、点击灯箱内的图、或按 `Escape`，灯箱关闭，页面恢复可滚动；
- 灯箱打开期间正文不可滚动（`body` 的 `overflow: hidden`）；
- 亮色主题下灯箱内的图与正文中一致；暗色主题下，开启了 `themeAdaptive` 的图在灯箱里**仍然带着反色滤镜**（类名没有卸掉）——如果反色图叠在深色遮罩上对比度变差，这是当前实现的既定行为。
**异常**：点击无反应；灯箱打开后无法关闭；`Escape` 无效；关闭后页面锁死无法滚动；灯箱层级被顶栏/侧栏盖住。
...
`XParser` 是 `index.js` 写法的遗留行内语法解析器（`` ` `` 行内代码、`*` 加粗、`$` 公式、`@文本[链接]@` 超链接、`\n` 换行）。markdown 文章**曾经**在"紧凑列表项内容全部是纯文本"时也走这条路径，产生与 markdown 语义不一致的二次解析畸变；自渲染层引入 `normalizeLiChildren` 将行内片段统一包装为 `<p>` 元素后，**markdown 文章的任何内容都不再触达 XParser**（它如今只服务于 `index.js` 写法的 `X.P` / `X.Uli` / `X.Oli` 纯字符串内容与 `X.Table` 的 `fromText`）。以下用例用于持续确认这一点，并记录仍然存在于 markdown 层的畸变（`remark-math` 与 GFM 自动链接）。

### 16.1 星号与美元符号

- 表达式 3 _ 4 _ 5 的结果是 60
- 售价从 $5 涨到 $10
- 单个星号 \* 不成对时的表现
- 单个美元 $ 不成对时的表现

**预期**：
第 1 项中 `* 4 *` **保持字面星号、不加粗**（XParser 已不介入，而 `* 4 *` 不满足 CommonMark 的强调规则）；
第 2 项中 `$5 涨到 $` **仍会**被 `remark-math` 在 markdown 层当作行内公式吃掉（这一步发生在渲染层之前，与 XParser 无关），渲染成数学排版的"5涨到"，后面孤零零跟着 `10` —— 正文里写美元符号应使用 `\$` 转义；
第 3、4 项的单个符号不成对，原样显示。
**异常**：第 1 项出现了加粗且星号消失（说明紧凑列表项又走回了 XParser 路径，检查 `normalizeLiChildren` 是否失效）；或页面报错。

### 16.2 单引号、@链接[]@ 语法与换行符字面量

- 反引号包裹 'pseudo-code' 的表现
- XParser 专有链接语法 @点我[https://example.com]@ 的表现
- 换行符字面量 \n 的表现

**预期**：第 1 项中的单引号只是普通字符；第 2 项里 GFM 仍会把 `https://example.com]@` 识别成自动链接、`@点我[` 以字面文本残留（markdown 层行为，与渲染层无关），但 XParser 的 `@文本[链接]@` 语法**不再生效**；第 3 项中的 `\n` **保持字面的反斜杠加 n**，不会被替换成 `<br/>` 换行。**异常**：`\n` 处出现了真实换行（说明 XParser 又被触发）；出现未闭合的 HTML 标签文本泄漏到页面上。

### 16.3 含行内元素的对照组

- 本项含有行内元素（例如这个 `代码`）：3 _ 4 _ 5 与 $5 到 $10 的表现应与 16.1 完全一致。

**预期**：markdown 的所有列表项如今**无论是否含行内元素**都统一走 React 渲染路径，本项与 16.1 的表现应当完全一致：`* 4 *` 保持字面星号不加粗；`$5 到 $` 仍被 `remark-math` 在 markdown 层解析为公式。**异常**：本项与 16.1 第 1 项的星号表现不一致（说明两条路径又出现了分叉）。

### 16.4 段落中的对照组

段落中的 3 _ 4 _ 5 与 $5 到 $10 与 'pseudo-code' 与 @点我[https://example.com]@ 与 \n。
...
_斜体（em 未映射）_、~~删除线（del 未映射）~~、以及行内 HTML <b>加粗标签</b> 与 <span style="color:red">带样式的 span</span>。
