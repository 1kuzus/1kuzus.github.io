...
全局基线（贯穿整页，不再逐条重复）：正文段落上下外边距 24px、行高 28px；代码块 / 引用块 / 表格 / 图片 / 公式 / 分割线的上下外边距统一为 24px；列表项上下外边距 12px、行高 28px。**异常**：任意两个相邻块级元素之间出现明显不等于 24px 的空隙（如 0px 挤在一起、或 48px 塌陷失败的双倍间距）。

本页需要在**亮色和暗色两种主题**下各走查一遍，并在**桌面（>1200px）、平板（600–1200px）、手机（<600px）** 三档视口下各走查一遍。
...
**预期**：`##` 渲染为 24px 标题且带一个品牌色 `#` 前缀；`###` 渲染为 20px 标题且带品牌色 `##` 前缀；`####` 渲染为 18px 标题且**没有**任何前缀符号。三级标题上下外边距均为 24px。**异常**：`#` / `##` 前缀是黑色/正文色而非品牌青色；`####` 也长出了前缀符号；三级标题字号看不出层级差异。
...
乙、结束标记前是右括号、后是汉字 → **桌面（1200px）**三档（失败）。

丙、结束标记前是右括号、后是顿号 → **桌面（1200px）**、三档（成功）。
...
**预期**：块级公式**水平居中**，上下外边距 24px，字号比行内公式大。**异常**：块级公式左对齐（说明 `.x-formula` 的 `text-align: center` 失效）；两个连续公式之间的间距不是 24px。
...
**预期**：块内三段之间间距 24px，但**第一段的上外边距和最后一段的下外边距为 0**（被 `:first-child` / `:last-child` 规则清零），所以文字到块边框的上下距离精确等于 24px 的内边距，不是 48px。**异常**：文字与块边缘之间的上下空隙明显大于左右内缩量的比例关系（说明首尾段落的外边距没被清零）。
...
**预期**：块内标题保留品牌色 `##` 前缀且首个标题的上外边距被清零；块内代码块的右上角标签**完整可见不被引用块裁切**（标签是 `top:-8px right:-8px` 的绝对定位，会略微超出代码块但仍在引用块的 32px 内边距范围内）。**异常**：块内标题也会进入右侧目录（目录只收 `.x-h1`/`.x-h2`，所以块内 `###` 会进目录 —— 这是**已知行为**，如果你不希望引用块内的标题进目录，这里就是需要修的地方）；代码块标签被引用块的圆角裁掉一角。
...
**预期**：两者**视觉上应当尽可能接近**（都是 12px 项间距）。这是本页最容易暴露差异的用例之一：紧凑列表的项内容是裸文本节点，松散列表的项内容是 `<p>` 元素，两条路径的外边距处理不同。**异常**：松散列表的项间距明显大于紧凑列表（例如 24px 或 36px），或松散列表首/末项与前后元素的间距塌陷。
...
**预期**：每深入一层，标记与内容整体右移一个标记区宽度（32px）；"有序套有序"中外层必须是 1/2/3、内层必须是 1/2（内层**不能**接着外层数成 2/3，外层第二项也**不能**因为内层而跳号变成 4）；四层嵌套的 L4 相对 L1 缩进 96px。**异常**：内外层编号串号；子列表与父项内容之间出现异常大的空隙；某一层的标记消失。
...
**预期**：同一项内的段落间距 24px，但首段上外边距与末段下外边距被清零；三个段落都在内容区列内（左边缘对齐）。**异常**：第二、三段跑到了标记正下方；项内段落间距与项间距（12px）混淆。
...
**预期**：每项内部呈"段落 → 代码块 → 段落"三段结构，垂直间距均匀（24px），整项外边距 12px。**异常**：代码块后的说明文字被吸进代码块或另起为新的列表项。
...
**预期**：单元格内的行内代码不撑破单元格；含图片的那一行行高被图片撑高但图片水平居中且上下外边距被清零（`td > :first-child` 规则）；链接为品牌色。**异常**：图片单元格上下出现 24px 的多余空隙（说明外边距清零规则失效）；行内代码换行时背景断裂。
...
**预期**：四张图都**水平居中**、上下外边距 24px、宽度不超过正文列（`max-width: 100%`）、高度按原比例自动缩放（不变形）。GIF 应当在播放动画。第三张图没有写 alt，其 `alt` 属性会被兜底为字符串 `image`。图片区域不可被文本选中（`user-select: none`）。**异常**：图片被拉伸变形（说明 `height: auto` 失效）；加载过程中页面发生跳动（CLS，说明宽高属性没生效）；超宽图片撑破正文列。
...
**预期**：**亮色主题下三张图应当完全一样**（滤镜只在 `:root.dark` 下生效）。切换到**暗色主题**后：第一张的白色背景变为深色、图中彩色元素色相保持大致可辨；第二张保持刺眼的白底；第三张与第一张相同（因为字符串 `"false"` 是 truthy 值，仍会加上反色类）—— 这是既定行为，`themeAdaptive` 只应作为布尔 flag 使用。**异常**：亮色主题下就出现了反色；暗色主题下反色图的文字/线条变得不可读。
...
**预期**：分割线为一条 1px 的细线，横跨正文宽度，上下外边距各 24px；两条连续分割线之间应有 24px 间距（呈现为两条平行细线）。**异常**：分割线是粗黑线；两条连续分割线粘在一起看起来像一条；分割线颜色在暗色主题下不可见。
