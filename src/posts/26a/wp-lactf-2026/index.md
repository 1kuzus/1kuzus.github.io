...
<!-- @xprops highlightLines="10-15,21" -->
...
利用YAML的[别名](https://yaml.org/spec/1.2.2/#71-alias-nodes)功能，可以使得`conf["blogs"][0]`和`conf["user"]`在解析后指向同一个字典对象：

<!-- @xprops highlightLines="2-5,8" -->
...
此时先验证`blog["name"]`时`.._/.._/flag`能够通过验证，随后该属性又在`display_name`处理后被修改为`../../flag`，从而实现LFI。
