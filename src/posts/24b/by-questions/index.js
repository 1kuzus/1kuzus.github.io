    ...
    return (
        <>
            ...
            <X.Oli reset>
                ...
                <X.P>
                    全概率公式：如果$A_1 \dots A_n$是对事件空间的划分，有{`$P(B)=\\sum_{i=1}^n P(A_i)P(B|A_i)$`}。
                </X.P>
                <X.P>
            ...
            <X.Oli>
                <X.P>什么是依概率收敛？</X.P>
                <X.P>
                    依概率收敛是指对于任意$\varepsilon \gt 0$，有$P(|X_n-X| \gt \varepsilon) \to
                    0$，则称$X_n$依概率收敛到$X$。
                </X.P>
            </X.Oli>
            <X.Oli>
            ...
            <X.Oli>
                ...
                <X.P>
                    CA：碰撞避免。CSMA/CA用于无线网络。无线网络存在“隐蔽站”问题，使用碰撞检测意义不大；CSMA/CA通过*预约信道*机制实现碰撞避免。考虑到无线信道通信质量远不如有线信道，CSMA/CA使用*确认帧*机制。
                </X.P>
            </X.Oli>
            <X.Oli>
            ...
            <X.Oli>
                <X.P>介绍一下ICMP协议？</X.P>
                <X.P>
                    全称是互联网控制报文协议。ICMP是网络层协议，从技术角度来说就是一个*差错报告机制*，不用来传递数据，而是用作网络诊断、排除故障等。一个常见的应用是`ping`命令。ICMP协议可能被恶意的用作*拒绝服务攻击*。
                </X.P>
            </X.Oli>
            <X.Oli>
                <X.P>介绍一下DHCP协议？</X.P>
            ...
            <X.Oli>
                ...
                <X.P>
                    TCP的发送方会维护发送窗口，接收方也会维护接收窗口，这点与SR类似；但TCP采用与GBN类似的累计确认。需要注意的是，GBN中`ACK
            ...
            <X.Oli>
                ...
                <X.P>
                    流量控制是端到端的问题，由接收方主动控制发送方的发送窗口，受限于接收方接收能力；拥塞控制是网络上的全局问题，由发送方主动控制发送速率，受限于网络拥塞程度。相同点是*二者控制的对象都是发送方的发送速率*。
                </X.P>
                <X.HighlightBlock>
            ...
            <X.Oli>
                ...
                <X.P>
                    WAN`(Wide Area Network)`口接外网，LAN`(Local Area
                    Network)`口接内网，也就是家里的设备。事实上如果不接WAN口，路由器就变成了交换机。
                </X.P>
                <X.P>
