...
export default function Post() {
    return (
        <>
            ...
            <X.P>
                *通用缺陷枚举*`(Common Weakness Enumeration,
                CWE)`是针对硬件和软件弱点和漏洞的分类系统。该项目由美国国土安全部（DHS）网络安全和基础设施安全局（CISA）办公室赞助，由MITRE公司运营。
            </X.P>
            <X.P>
            ...
            <X.P>
                下面的内容译自@CWE
                Glossary[https://cwe.mitre.org/documents/glossary/]@，这里只列出了几个下文可能出现的术语：
            </X.P>
            <X.Uli>
            ...
            <X.Uli>
                <X.P>`Resource`（资源）：</X.P>
                <X.P>
                    指在产品运行过程中被访问或修改的对象或实体，例如内存、CPU、文件或套接字。资源可以是系统级（内存或CPU）、代码级（函数或变量）或应用程序级（cookie或消息）。
                </X.P>
            </X.Uli>
            <X.Uli>
                <X.P>`Property`（属性）：</X.P>
                <X.P>
                    指某个对系统预期的安全模型很重要的资源或行为，可能随时间变化。比如用户的输入最初是不受信任的，但经过系统的中和处理后，最终会被认为是受信任的。
                </X.P>
            </X.Uli>
            <X.H2>CWE分类系统</X.H2>
            ...
            <X.P>
                CWE的不同条目也具有不同的抽象程度，有些条目描述了具体的缺陷，有些则描述了更一般的问题。CWE的条目之间也存在父级子级的关系。四种抽象程度如下图所示：
            </X.P>
            ...
            />
            <X.P>
                `returnChunkSize()`遇到错误会返回`-1`，但是`memcpy()`假定传入的参数是`unsigned`，此时负值会被转换成相当大的数值。
            </X.P>
            <X.H3>Example 3</X.H3>
            <X.CodeBlock
                language="c"
            ...
            />
            <X.P>
                虽然前面做了长度检查，但是后续的逻辑将`'&'`和`'&lt;'`转为HTML实体时，可能导致长度增长。如果恶意构造包含大量`'&'`的输入，可能导致越界写入。
            </X.P>
            ...
            />
            <X.P>
                由于cookie是可以在本地修改的，这仍然是一个输入用户可控的例子。注意到PHP语句中的单引号包裹着`$id`，一个可以注入的载荷是`1432'
                or '1' = '1`。请求会变成：
            </X.P>
            <X.CodeBlock
                language="sql"
            ...
            />
            <X.P>
                这段程序展示用户的生日和主页，但由于这些参数来自于HTTP请求，攻击者可以构造一个含有`script`标签的钓鱼链接，并通过参数传入，导致恶意脚本在客户端上执行。
            </X.P>
            ...
            <X.HighlightBlock background="blue">
                ...
                <X.P>
                    ...
                    2.6中的`group_posts.php`中的SQL注入漏洞，远程攻击者可以通过`tid`参数执行任意SQL命令。
                </X.P>
                <X.CodeBlock
                    language="perl"
                    highlightLines="16"
            ...
            />
            <X.P>
                上述代码会导致“绝对路径遍历”，`os.path.join`如果第二个参数传入绝对路径，则会弃用第一个参数`os.getcwd()`，直接使用第二个参数`filename`。那么传入`/etc/passwd`就可能导致同样的攻击。
            </X.P>
            <X.P>一个正确的做法是：</X.P>
            <X.CodeBlock
                language="python"
            ...
            <X.HighlightBlock background="blue">
                ...
                />
                <X.P>
                    进一步去查找`safeEntryName()`的功能，溯源到@PathTraversalChecker.java[https://github.com/gradle/gradle/blob/master/platforms/core-runtime/files/src/main/java/org/gradle/internal/file/PathTraversalChecker.java]@中的以下代码：
                </X.P>
                <X.CodeBlock
                    language="java"
            ...
            <X.HighlightBlock background="blue">
                ...
                <X.P>
                    在bingo!CMS
                    1.2及更早版本中存在CSRF漏洞，远程攻击者可劫持其他用户对修改配置或更改内容请求的身份验证。
                </X.P>
            </X.HighlightBlock>
            ...
            <X.HighlightBlock>
                ...
                <X.Uli>
                    `Authentication
                    (AuthN)`，身份认证，通常在授权之前完成，是验证“用户是谁”的过程。一般通过账号密码、人脸识别的方式。\n例如：员工通过输入用户名和密码登录公司网络。
                </X.Uli>
                <X.Uli>
                    `Authorization
                    (AuthZ)`，授权，是确认用户是否有权访问资源。一般通过角色规则（管理员、普通用户）等方式。\n例如：普通员工登录公司网络后，不能访问公司较为敏感的数据。
                </X.Uli>
            </X.HighlightBlock>
            <X.H3>Example 1</X.H3>
            <X.CodeBlock
            ...
            <X.HighlightBlock background="blue">
                ...
                <X.P>
                    ...
                    CD版本都容易受到路径遍历错误的影响，再加上不适当的访问控制错误，可能允许具有只读存储库访问权限的恶意用户从Argo
                    CD的存储库服务器中泄露敏感文件。
                </X.P>
                <X.P>
            ...
            <X.P>
                整数溢出或整数回绕，当有符号数相加超过正负值上限时可能发生溢出；当无符号数相加超过上限就会从`0`开始“回绕”；或者计算`unsigned(0)
                - 1`时，会得到最大值。
            </X.P>
            <X.H3>Example 1</X.H3>
            <X.CodeBlock
            ...
            <X.P>
                `bytesRec`定义为`short
                int`类型，当`MAXGET`很大时，很可能导致`bytesRec`溢出（永远小于`MAXGET`），循环不会终止，并不断地覆盖`buf`中的内容。
            </X.P>
            <X.HighlightBlock background="blue">
                <X.H3>Example in real-world project</X.H3>
            ...
            />
            <X.P>
                这段代码没有验证传入数据是否合法。攻击者可以构造一个非法的序列化对象`AuthToken`，该对象通过实例化Python的子进程之一来执行任意命令。例如，攻击者可以构建一个利用Python子进程模块的`pickle`，该模块会生成新进程并包含许多用于各种用途的参数。由于`Pickle`库允许对象定义如何`unpickle`，因此攻击者可以指示`unpickle`进程在子进程模块中调用`Popen`并执行`/bin/sh`。
            </X.P>
            <X.HighlightBlock background="blue">
                <X.H3>Example in real-world project</X.H3>
                <X.Uli>OpenCVE：@CVE-2020-13091[https://www.opencve.io/cve/CVE-2020-13091]@</X.Uli>
                <X.P>
                    如果`__reduce__`调用了`os.system`，1.0.3版本的Python
                    Pandas库可以从传递给`read_pickle()`函数的不信任文件中反序列化，并执行命令。注意`read_pickle()`在@文档[https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.read_pickle.html]@中已经被标记为不安全的，用户有责任确保传入的文件是可信的。
                </X.P>
            </X.HighlightBlock>
            ...
            <X.HighlightBlock background="blue">
                ...
                <X.P>
                    ...
                    这是更安全的。\n使用`shell=True`时，命令通过解释器（例如`/bin/sh`或`cmd.exe`）运行，例如：`subprocess.run('ls
                    -l', shell=True)`；这可能导致命令注入。
                </X.P>
            </X.HighlightBlock>
            ...
            />
            <X.P>
                有权限访问到此源码的员工可以利用这点闯入系统。更坏的情况是如果攻击者能够访问到程序的字节码，可以使用`javap
                -c`命令反汇编，得到明文密码，例如：
            </X.P>
            <X.CodeBlock
                language="text"
            ...
            <X.HighlightBlock background="blue">
                ...
                <X.P>
                    AnythingLLM的上传链接功能存在SSRF漏洞，该功能面向具有`manager`或`admin`角色的用户，使用无头浏览器通过内部`collectorApi`处理上传的链接。
            ...
            <X.HighlightBlock background="blue">
                ...
                <X.P>
                    Sectona Spectra 3.2.0存在一个易受攻击的SOAP
                    API端点，该端点会在未进行适当身份验证的情况下泄漏敏感信息。
                </X.P>
            ...
            <X.Uli>
                ...
                <X.P>
                    程序员可能会认为某些代码序列执行得太快，不会受到干扰代码序列的影响，但事实并非如此。例如，单个`x++`语句在代码层可能看起来是原子的，但在指令层实际上是非原子的，因为它涉及读取`x`的原始值、计算`x+1`、最后将结果赋值给`x`。
                </X.P>
            </X.Uli>
            <X.P>
            ...
            />
            <X.P>
                上面的函数尝试获取锁，以便对共享资源执行操作。不过上述函数没有检查`pthread_mutex_lock()`是否正常返回。如果因为一些原因导致`pthread_mutex_lock()`失败，那么这个例子会导致数据竞争。
            </X.P>
            <X.P>正确的做法是：</X.P>
            <X.CodeBlock
                language="c"
            ...
            />
            <X.P>
                上述代码先提高权限以创建文件夹，然后立刻降回来。但如果`os.mkdir()`发生了异常，就不会调用`lowerPrivileges()`，这时程序会在提高的权限下运行，可能导致进一步被利用。
            </X.P>
            <X.H3>Example 2</X.H3>
            <X.CodeBlock
                language="java"
            ...
            />
            <X.P>
                这个系统定义了不同等级的用户，`ADMIN`和`OPERATOR`都可以重置密码。系统设计的初衷是想让`OPERATOR`拥有稍弱于`ADMIN`的权限；然而既然`OPERATOR`可以修改密码，那么就可以通过重置`ADMIN`账号的密码来控制一个管理员账号。此例的权限管理是有缺陷的。
            </X.P>
            <X.HighlightBlock background="blue">
                <X.H3>Example in real-world project</X.H3>
                ...
                <X.P>
                    Tinyproxy
                    1.8.4及更早版本中的`main.c`会在向非`root`帐户下放权限后创建`/run/tinyproxy/tinyproxy.pid`文件，这可能允许本地用户在`root`脚本执行`kill
                    \`cat
                    /run/tinyproxy/tinyproxy.pid\``命令之前，利用访问该非`root`帐户的权限来修改`tinyproxy.pid`，从而杀死任意进程。
                </X.P>
                <X.CodeBlock
                    language="c"
            ...
            <X.HighlightBlock background="blue">
                ...
                <X.P>
                    ...
                    FAQ`功能可以给其他邮箱分享讯息，前端只有`5`个输入框（希望用户最多分享给`5`个邮箱），然而后端并没有对数量做限制，攻击者可以构造请求发送大量邮件。
                </X.P>
                <X.P>部分补丁的内容：</X.P>
                <X.CodeBlock
                    language="php"
            ...
            <X.HighlightBlock background="blue">
                ...
                <X.P>
                    0.9.5-r1版本之前的SilverCity安装了`cgi-styler-form.py`、`cgistyler.py`和`source2html.py`，具有`world
                    writable`权限。
                </X.P>
                <X.CodeBlock
                    language="text"
