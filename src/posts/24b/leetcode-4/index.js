...
export default function Post() {
    return (
        <>
            ...
            <X.Oli>
                算法终止时，短数组区间长度可能是`1`或`2`个元素，最终的结果就是把这两个元素插入到长数组区间中，再取中位数。为了保证效率，不去做插入，而是讨论长数组区间中位数附近的元素。
            </X.Oli>
            <X.P>
                注意上述第三点，假设长数组区间中位数情况为：`[... a m b ...]`或`[... a m1 m2 b
                ...]`，那么如果`a`和`b`存在（没越界）也要纳入考虑。这是因为如果短数组剩余的元素均小于`a`，则有可能中位数需要通过`a`和`m`（或`m1`）计算得到。反之亦然。体现在代码中，短数组可能向`ans`贡献`1`或`2`个元素，长数组可能向`ans`贡献`1`~`4`个元素。最后对至多含有`6`个元素的`ans`排序取中位数即可。
            </X.P>
            <X.CodeBlock
                language="cpp"
                highlightLines="22"
