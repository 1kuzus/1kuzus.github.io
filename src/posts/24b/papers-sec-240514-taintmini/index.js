...
export default function Post() {
    return (
        <>
            <X.P>相关链接：</X.P>
            <X.Uli>
                @TAINTMINI: Detecting Flow of Sensitive Data in Mini-Programs with Static Taint
                Analysis[https://chaowang.dev/publications/icse23.pdf]@
            </X.Uli>
            ...
            <X.Uli>
                `Data
                Node`：数据流分析的最小粒度，可能是一个JS对象或WXML标签；生成方式是通过给每个页面JS文件生成AST，此过程会合并通过模块化导出/导入但最后组成同一页面的文件。
            </X.Uli>
            <X.Uli>
                `Event
                Group`：事件组有一个非常重要的性质，就是*事件组的代码同步执行*；同一个JS对象节点可以属于不同的事件组。
            </X.Uli>
            ...
            <X.Uli>
                第三条{`$S_p^{(i,n)}$`}表示一条语句$S_p$（原文是`p-th JavaScript code
                statement`，但$p$的数值并不重要）属于事件组$e_i$，并且由于事件组同步执行，一定会存在一个语句被执行的次序$n$；
            </X.Uli>
            <X.Uli>第四条也好理解；</X.Uli>
            <X.Uli>
            ...
            <X.Uli>
                ...
                <X.P>
                    这个例子中{`$S_p^{(i,n)}$`}
                    就是`request`函数，后续的`console.log`语句就是$e_i$中执行次序大于$n$的语句，它们会与$e_j$异步执行。
                </X.P>
            </X.Uli>
            <X.P>然后是Rules：</X.P>
            <X.Uli>
            ...
            <X.Uli>
                ...
                <X.P>
                    在两条规则、分别$n \lt m$和$n \gt
                    m$共四种可能的情况中，有一种情况是必然会有$O_a$到$O_c$的数据流动，两种情况是可能会有（取决于异步执行的情况），只有一种情况是不可能发生数据流动的；根据前面提到的乐观原则，仅将这一种情况排除在外（也就是第六条规则但$n
                    \gt m$的情况）。
                </X.P>
            </X.Uli>
            <X.Uli>
            ...
            <X.P>
                数据集来源于爬取并经过查重后的`238866`个小程序。精度方面，人工分析了`100`个小程序作为Ground
                Truth，其中有`0%`的`FP`，`5%`的`FN`（由于用户使用了非官方
                API，或直接从`input`收集手机号）。性能方面，平均运行时间为`3.73s`，检测出`11.38%`存在敏感数据流。
            </X.P>
