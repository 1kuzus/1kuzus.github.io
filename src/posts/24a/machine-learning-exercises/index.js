    ...
    return (
        <>
            ...
            <X.Oli>
                ...
                <X.Uli>
                    `ReLU`：计算简单，加速神经网络的收敛；可以缓解梯度消失的问题；但有可能导致神经元“死亡”（如果`ReLU`层的输入总是负的，那么该处梯度一直为$0$，神经元无法学习）；在现代神经网络架构中极为普遍。
                </X.Uli>
            </X.Oli>
            <X.Oli>
            ...
            <X.Oli>
                <X.P>
                    BN层有两个可学习参数：拉伸参数$\gamma$和偏移参数$\beta$，它们的形状与$x$相同。请写出经过批量归一化转换后的$x$表达式。
                </X.P>
                <X.P>答：</X.P>
            ...
            <X.Oli>
                ...
                <X.P>答：</X.P>
                <X.Uli>
                    模拟集成学习：在训练时，Dropout随机关闭网络中的一部分神经元，可以被看作是在训练每个小批量数据时都使用了一个略有不同的网络结构，这类似于训练多个不同的模型并进行集成学习。在测试时，使用所有神经元，相当于对这些“模型”进行平均，这有助于提高泛化能力。
                </X.Uli>
                <X.Uli>
                    减少复杂共适应性：在没有Dropout的情况下，网络的神经元可能会学会共同适应并对特定的训练数据过度拟合，即它们共同调整它们的行为来记住训练数据的特定特征。使用Dropout意味着网络的神经元不能依赖于特定其他神经元的存在，因此，每个神经元必须学习更加鲁棒的特征，这些特征在各种不同的网络子集中都有用。
                </X.Uli>
                <X.Uli>
            ...
            <X.Oli>
                ...
                <X.Uli>
                    内部协变量偏移：批归一化通过规范化激活层的输出来减少内部协变量偏移。但是，当使用Dropout时，由于随机关闭神经元，激活层的输出分布在每次迭代中都会发生变化，这可能会破坏批归一化试图解决的问题。
                </X.Uli>
                <X.Uli>
                    影响训练：当同时使用这两种技术时，网络的训练可能会受到干扰。Dropout会改变网络中信息的流动方式，而批归一化依赖于这些信息的稳定分布来进行有效的规范化。
                </X.Uli>
                <X.Uli>
                    测试时行为不一致：当网络的状态从训练转移到测试时，以$p$概率关闭神经元的Dropout层计算时权重会乘$(1-p)$，改变了输出的方差，而测试阶段BN保持了它的统计方差。这种方差的不一致性会影响预测的结果。
                </X.Uli>
            </X.Oli>
            <X.Oli>
            ...
            <X.Oli>
                <X.P>
                    设模型初始参数$\theta_0=(0.1,0.2)^T$，学习率为$\eta=0.1$，动量更新权重为$\lambda=1$，若模型在两次训练过程中的梯度依次为$g_0=(2,-1)^T$、$g_1=(1,-2)^T$，求两次训练后模型的参数$\theta_2$。
                </X.P>
            ...
            <X.Oli>
                <X.P>
                    Batch
                    Normalization中反向传播的过程涉及到计算对于输入的梯度，该梯度计算通常需要计算均值和方差的（）。
                </X.P>
                <X.P>答：梯度。</X.P>
            </X.Oli>
            <X.Oli>
            ...
            <X.Oli>
                ...
                <X.P>
                    答：在最大池化中，反向传播时梯度会被传递给池化层输入中最大值所对应的位置，而其他位置的梯度为零；在平均池化中，反向传播时梯度会均匀分配给池化层输入的所有元素。
                </X.P>
            </X.Oli>
            <X.Oli>
            ...
            <X.Oli>
                ...
                <X.Uli>
                    VGG：提出了可以通过重复使用简单的基础块来构建深度模型的思路。使用三个$3 \times 3$卷积核代替$7 \times
                    7$卷积核，使用两个$3 \times 3$卷积核代替$5 \times
                    5$卷积核，在保证具有相同感知野的条件下，提升了网络的深度，减小了网络参数，在一定程度上提升了神经网络的效果。
                </X.Uli>
                ...
                <X.HighlightBlock background="red">
                    <X.P>
                        *门控循环单元*`(Gated Recurrent Unit,
                        GRU)`通过简化LSTM神经网络循环函数达到了类似的效果并节省了计算成本。在GRU中，遗忘门和输入门合并成了一个新的重置门，且加入了一个更新门。
                    </X.P>
                </X.HighlightBlock>
            </X.Oli>
            <X.Oli>
            ...
            <X.Oli>
                ...
                <X.Uli>
                    <X.P>
                        梯度消失，JS散度的问题：只要两分布没有重叠部分或重叠部分可忽略，JS散度恒为$\log2$，目标函数为$0$。当判别器最优时，生成器无法获得任何梯度信息，梯度消失，无法训练。
                    </X.P>
                    <X.P>改进办法：</X.P>
                ...
                <X.Uli>
                    ...
                    <X.P>改进办法：</X.P>
                    <X.Oli reset>
                        从目标函数考虑：经验发现，当GAN出现模式崩溃问题时，通常判别器在真实样本附近更新参数时，其梯度值非常大。可对判别器在真实样本附近施加梯度惩罚项。试图在真实样本附近构建线性函数，因为线性函数为凸函数具有全局最优解。（DRAGAN）
                    </X.Oli>
                    <X.Oli>
                        从网络架构考虑：即使单个生成器会产生模式崩溃的问题，但是如果同时构造多个生成器，且让每个生成器产生不同的模式，则这样的多生成器结合起来也可以保证产生的样本具有多样性。（Multi-Agent
                        Diverse GAN）
                    </X.Oli>
                </X.Uli>
                <X.P>一些其他的改进：</X.P>
                <X.Uli>
                    Wasserstein
                    GAN：提出是为了解决传统GAN遇到的梯度消失、训练时梯度不稳定以及模式崩溃等问题。使用Wasserstein距离来衡量两分布之间的距离，抛弃了KL散度与JS散度。
                </X.Uli>
                <X.Uli>
            ...
            <X.Oli reset>
                ...
                <X.HighlightBlock background="red">
                    ...
                    <X.Uli>
                        *回报*`(return)`：在$t$时刻的*折扣回报*是{`$U_t=R_t+\\gamma R_{t+1}+\\gamma^2 R_{t+2}+\\dots$`}
                    </X.Uli>
                    <X.Uli>
            ...
            <X.Oli>
                <X.P>
                    驾车按照“甲、乙、丙”行驶，从甲地出发，模型预计需要行驶$20$小时，实际行驶$6$小时到达乙地，模型预计还需$12$个小时到达丙地，如果我们用TD算法更新模型，TD目标$y$小时，TD绝对误差值$|\delta|$小时。$y$和$|\delta|$是多少？
                </X.P>
            ...
            <X.Oli>
                ...
                <X.P>
                    答：基于相似的样本具有相同标签的假设，构建图结构，利用节点标签扩散，将已标记样本根据样本特征的相似性进行扩散，获得未标记样本的标记。过程包括：图表示、初始化标签、标签传播、学习模型、分类预测。
                </X.P>
            </X.Oli>
            <X.Oli>
            ...
            <X.Oli>
                ...
                <X.P>
                    答：“伪标签”是一种半监督学习中的策略，通过使用已训练模型对未标记数据进行预测，并将这些预测结果作为“伪标签”来扩充训练数据。优点是可以扩充训练数据，引入额外信息；但是要注意误差传播的问题。
                </X.P>
            </X.Oli>
            <X.Oli>
            ...
            <X.Oli reset>
                ...
                <X.P>
                    答：迁移学习是指利用数据、任务或模型之间的相似性，将在旧领域学习过的模型，应用于新领域的一种学习过程。迁移学习提供了一种将预训练模型在自己数据集上进行微调的技术。核心思路是找到源域和目标域之间的差异性，并将整体工作归为两方面：一是度量两个领域的相似性，不仅定性，还能定量的给出相似程度；二是以度量为准则，通过我们所采用的学习手段增大两个领域之间的相似性，完成迁移学习。
                </X.P>
            </X.Oli>
            <X.Oli>
            ...
            <X.Oli>
                ...
                <X.P>
                    答：领域自适应旨在解决在源域上训练的模型在目标域上性能下降的问题。在领域自适应中，源域和目标域通常具有不同的分布或特性，例如，它们可能来自不同的传感器、环境或任务。
                </X.P>
                <X.P>基本原理包括：</X.P>
                <X.Uli>源域训练：在源域上使用标记的数据训练一个模型。</X.Uli>
                <X.Uli>
                    领域间差异：源域和目标域存在领域间的差异，这些差异可能包括数据分布的不同、特征的变化等。由于这些差异，直接将源域上训练的模型应用到目标域上可能导致性能下降。
                </X.Uli>
                <X.Uli>
            ...
            <X.Oli>
                ...
                <X.P>
                    答：源域是模型在训练阶段接触到的领域，通常具有相对丰富的标记数据。目标域是模型在测试阶段面对的领域，通常没有或有很少的标记数据。将源域和目标域的差异性视为两个域内样本数据的概率分布的差异性进行研究，根据采用显式或隐式的距离度量分为两类：
                </X.P>
                <X.Uli>
            ...
            <X.Oli reset>
                ...
                <X.P>
                    答：相似度函数是用于度量两个对象之间相似程度的函数。在机器学习和数据挖掘中，相似度函数通常用于比较两个样本、文档、图像或其他数据表示的对象之间的相似性。相似度函数的设计直接影响了模型的性能，因为它定义了模型对相似性的理解。在迁移学习中，相似度函数可以用于度量源域和目标域之间的相似性，或者度量不同任务之间的相似性。基本思想是从大规模训练数据集中学习一个相似度函数，将相似度函数用于预测，通过对比查询样本与支持集每一个样本的相似度，找到得分最高的那个。
                </X.P>
            </X.Oli>
            <X.Oli>
                ...
                <X.P>
                    孪生网络是一种特殊的神经网络结构，有两个相同的子网络，它们共享相同的权重。这两个子网络分别处理输入样本，然后通过一个比较层将两个子网络的输出进行比较。孪生网络的训练目标是使得相似的样本在比较层中的距离较小，而不相似的样本的距离较大。对比损失是孪生网络中常用的损失函数。
                </X.P>
                <X.P>
                    三元组损失是一种用于训练相似度学习模型的损失函数，通常与三元组网络一起使用。在一个三元组中，包含一个锚点样本、一个正样本和一个负样本。三元组损失的目标是通过拉近锚点样本和正样本之间的距离，并推远锚点样本和负样本之间的距离，从而学到一个能够准确衡量相似性的模型。
                </X.P>
            </X.Oli>
        </>
    );
