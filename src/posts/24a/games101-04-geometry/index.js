...
export default function Post() {
    return (
        <>
            ...
            <X.P>构造实体几何`(Constructive Solid Geometry, CSG)`通过基本几何体的布尔运算（交、并、差等）来定义新的几何体。</X.P>
            <X.Image src="fig2.jpg" width="100%" />
            <X.H2>有符号距离函数 - SDF</X.H2>
            <X.P>有符号距离函数`(Signed Distance Function, SDF)`定义空间中一点到要描述的表面的最短距离，如果点在物体内部，则这个距离取负数。</X.P>
            <X.H3>合并和相交</X.H3>
            <X.P>通过对两个SDF函数的运算，得到一个新的SDF函数，再求出这个新SDF函数值为$0$的部分，即可得到组合后的图形。</X.P>
            <X.Image src="fig3.png" width="800px" />
            <X.P>要注意的是，这个结果并不是完全准确的，例如下图中的黄点的组合SDF值为两条黄色线段的最小值，但其准确值应为绿色线段的长度。出现这个问题的原因是，计算得到原图形SDF值的边界在新图形中已经不存在了。不过在应用中我们可以忽略这个小问题。</X.P>
            ...
            <X.FlexRow gap="32px">
                <X.Image src="fig6.jpg" width="300px" themeAdaptive />
            ...
            <X.P>这是Wavefront科技开发的一种描述三维对象的文件格式，是一个文本文件，通常包含顶点`v`，法线`vn`，面`f`等定义信息；例如下面的代码表达了一个四面体：</X.P>
            <X.CodeBlock
                language="text"
                code={`
                # 顶点
            ...
            <X.H2>贝塞尔曲线</X.H2>
            <X.Image src="fig8.jpg" width="400px" themeAdaptive />
            ...
            <X.Image src="fig9.jpg" width="600px" themeAdaptive />
            ...
            <X.FlexRow gap="32px">
                <X.Image src="fig10.jpg" width="360px" themeAdaptive />
                <X.Image src="fig11.jpg" width="360px" themeAdaptive />
            </X.FlexRow>
            ...
            <X.Image src="fig12.jpg" width="600px" themeAdaptive />
            ...
            <X.Image src="fig13.jpg" width="600px" themeAdaptive />
            <X.H3>Loop细分</X.H3>
            <X.HighlightBlock>
            ...
            <X.Image src="fig16.jpg" themeAdaptive />
            <X.P>对于`边点`，其取法为：{`$e=\\frac{f_1+f_2+v_1+v_2}{4}$`}</X.P>
            <X.Image src="fig17.jpg" themeAdaptive />
            <X.P>
                第二步，对于原来的顶点$p$，其调整为：{`$v=\\frac{f_1+f_2+f_3+f_4+2(m_1+m_2+m_3+m_4)+4p}{16}$`}
                \n这里的$m$是边的中点，与上面求得的`边点`不同。
            </X.P>
            <X.Image src="fig18.jpg" themeAdaptive />
            <X.H2>网格简化</X.H2>
            <X.P>网格简化`(mesh simplification)`相当于下采样。</X.P>
            <X.Image src="fig19.jpg" themeAdaptive />
            <X.H3>边坍缩</X.H3>
            <X.P>边坍缩合并一条边的两个顶点。</X.P>
            <X.Image src="fig20.jpg" width="600px" themeAdaptive />
            <X.P>如何评定新的顶点与原模型轮廓一致的程度呢（例如下面左图取平均的效果明显不如右图）？答案是优化*二次误差*，也就是新的顶点与原来的各面的距离平方之和最小。</X.P>
            <X.Image src="fig21.jpg" width="600px" themeAdaptive />
            <X.P>执行算法时，对于模型的每一条边，都可以算出坍缩后最优的二次误差；使用*贪心算法*，每次取二次误差最小的边进行坍缩。注意到，将一条边坍缩后，会影响与这条边相邻接的其他边的位置，自然也会影响其他边的最优二次误差值，因此每次操作后还需要调整部分边的最优二次误差值。可以利用*优先队列*实现功能。</X.P>
        </>
    );
}
