...
export default function Post() {
    return (
        <>
            ...
            <X.P>
                隐式`(implicit)`表示就像数学中的隐函数，比如$f(x,y)=0$、$g(x,y,z)=0$，通过描述点满足的条件来表达几何体；这样的方式容易验证一点$(x_0,y_0,z_0)$是否在定义的面上，但求出构成形体的所有点是困难的，因为这需要求出方程的解。
            </X.P>
            <X.P>
            ...
            <X.P>
                这是Wavefront科技开发的一种描述三维对象的文件格式，是一个文本文件，通常包含顶点`v`，法线`vn`，面`f`等定义信息；例如下面的代码表达了一个四面体：
            </X.P>
            <X.CodeBlock
                language="text"
                code={`
            ...
            <X.P>
                对于三阶贝塞尔曲线上一点$b_0^3$，也就是图中标识的$x(t)$，尽管从演示上是通过中间点一步一步得到的，但实际给出四个控制点位置和$t$就可以唯一确定。
            </X.P>
            <X.P>
                下图中左侧是常见的分段定义贝塞尔曲线的方法，每个顶点都延伸出两根“控制杆”，而实际上三阶贝塞尔曲线的四个控制点是图中标出的$b_0$~$b_3$，相当于省去了连线$b_1b_2$。
            </X.P>
            ...
            <X.P>
                生成贝塞尔曲面的方式类似双线性插值：我们需要`16`个控制点排列成$4 \times
                4$，对于四个点组成的一列可以生成一个贝塞尔曲线（图中灰色线），指定了参数$u$后，可以分别得到四条曲线上的四个点（图中蓝色点）；以这四个蓝色点作为新的控制点，每一个参数$v$都会给出蓝色贝塞尔曲线上的一点（图中白色点）。
            </X.P>
            ...
            <X.P>
                执行算法时，对于模型的每一条边，都可以算出坍缩后最优的⼆次误差；使用*贪心算法*，每次取二次误差最小的边进行坍缩。注意到，将一条边坍缩后，会影响与这条边相邻接的其他边的位置，自然也会影响其他边的最优二次误差值，因此每次操作后还需要调整部分边的最优二次误差值。可以利用*优先队列*实现功能。
            </X.P>
        </>
    );
}
