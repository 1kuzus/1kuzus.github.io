...
export default function Post() {
    return (
        <>
            ...
            <X.P>从图中还可以看出，一定不会有`12`、`24`、`22`、`11`、`21`、`14`这六个组合出现，因为它们无法被`4`或`6`生成。</X.P>
            <X.P>注意图中有两个环，在被`6`生成的数中有一个长度为`6`的环，在被`4`生成的数中有一个长度为`2`的环。</X.P>
            <X.H2>矩阵等比数列求和</X.H2>
            <X.P>这一步应该是我和绝大部分题解做的不一样的地方，大多数题解都是将`1`位或`2`位的状态合在一个大矩阵里考虑的。我自己做的时候就想都往最初的$4 \times 4$矩阵上靠，所以后面扯到了一些比较难搞的数学操作。</X.P>
