...
export default function Post() {
    return (
        <>
            ...
            <X.P>
                对于这道题，$S_0=[1,0,0,0]$，它和$G^t$相乘实际上得到的是$G^t$的第一行。这也是题目中查询字符串长度为`1`时的解。对于$G^t$的求解，由于$t
                \lt 10^9$，需要使用快速幂求解。
            </X.P>
            <X.H2>两位状态转移</X.H2>
            <X.P>查询字符串长度为`2`时，它们之间的生成关系是：</X.P>
            ...
            <X.H2>矩阵等比数列求和</X.H2>
            <X.P>
                这一步应该是我和绝大部分题解做的不一样的地方，大多数题解都是将`1`位或`2`位的状态合在一个大矩阵里考虑的。我自己做的时候就想都往最初的$4
                \times 4$矩阵上靠，所以后面扯到了一些比较难搞的数学操作。
            </X.P>
            <X.P>
