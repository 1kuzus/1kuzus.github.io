...
export default function Post() {
    return (
        <>
            ...
            <X.P>
                第二次记录*单程时间*，也就是从当前酒店出发遍历子树所有的需要某种食材的酒店，等待时间最长的酒店等待的最短时间。\n考虑单程时间相比往返时间减少的部分，假如从当前节点$root$选择的最后走一个子树分支$i$，就不需要再从此路返回了。因此首先节省了一倍$w(root,i)$的时间；其次，对于遍历这棵子树花费的时间，也从往返时间变成了单程时间，因此还可以节省
            ...
            <X.H2>线段树懒标记优先级</X.H2>
            <X.P>线段树题中有一类就是会有多个懒标记，需要我们指定优先级，例如经典的“先乘后加”@洛谷P3373【模板】线段树2[https://www.luogu.com.cn/problem/P3373]@。此题涉及了三种运算：乘、加、左移，需要保证先乘后加，左移的优先级任意。这里以*左移优先级最后*为例分析懒标记的合并。</X.P>
            <X.HighlightBlock>
                <X.H3>个人理解</X.H3>
