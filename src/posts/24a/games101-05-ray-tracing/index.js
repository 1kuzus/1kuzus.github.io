...
export default function Post() {
    return (
        <>
            ...
            <X.HighlightBlock>
                <X.P>
                    @Möller-Trumbore[https://en.wikipedia.org/wiki/Möller–Trumbore_intersection_algorithm]@算法可以快速求解三维空间中光线与三角形的交点（也能判断是否有交点、交点是否在三角形内）。
                </X.P>
            </X.HighlightBlock>
            <X.H1>加速光线求交</X.H1>
            <X.H2>光线与轴对齐包围盒（AABB）求交</X.H2>
            <X.P>
                三维的轴对齐包围盒`(Axis-Aligned Bounding Box,
                AABB)`是一个长方体，其六个面都与坐标轴平行且能够刚好包裹住物体。加速光线与物体求交的一个自然想法是，先判断光线是否与物体的AABB相交，如果不相交则光线不可能与物体相交。
            </X.P>
