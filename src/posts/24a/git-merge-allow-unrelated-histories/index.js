...
export default function Post() {
    return (
        <>
            ...
            <X.P>上面的参数`-X theirs`是合并策略，在将`master`分支合并到`v1`分支发生冲突时选择`master`分支的更改。现在，两个分支的提交记录都被保留了，但是目录下会多出仅在`master`分支中存在的文件，删掉它们即可。或者更简单地，直接清空目录，把`master`分支中的内容复制过来。</X.P>
            <X.P>接下来在执行`git add`、`git commit`之后提交这次删除操作，也就是把本地的`v1`分支提交到远程的`master`：</X.P>
            <X.CodeBlock
                language="bash"
                code={`
                ...
                `}
            />
            <X.P>最后来到之前的开发目录，拉取最新的`master`分支。这步操作实际上并没有改变文件内容，只是多了提交记录。</X.P>
            <X.CodeBlock
                language="bash"
                code={`
                cd path/to/work_dir
