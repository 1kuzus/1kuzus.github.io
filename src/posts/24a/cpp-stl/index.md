...
- `str.replace(pos,n1,n2,ch)`：将`str`从下标`pos`开始连续的`n1`个字符替换成`n2`个`ch`。

- `str1.replace(pos,n,str2)`：将`str1`从下标`pos`开始连续的`n`个字符替换成`str2`。
...
- `str.erase(pos,n)`：将`str`从下标`pos`开始的`n`个字符删除。
...
- `str.substr(pos)`：取`str`从下标`pos`开始到结尾的字符串。

- `str.substr(pos,n)`：取`str`从下标`pos`开始的`n`个字符。
...
- `str.find(sub)`：在`str`中查找`sub`第一次出现的位置。

- `str.find(sub,pos)`：在`str`中从下标`pos`开始查找`sub`第一次出现的位置。
...
手写时需要注意一个细节：如果字符串是全`0`，则需要至少保留一个`0`。
...
也可以与`string`一样使用下标、迭代器和`auto`关键字访问。
...
- `insert(it,x)`：在迭代器`it`指定的位置之前插入新元素`x`，并返回新插入元素位置的迭代器。
...
vector可以看作是一个动态数组，会动态的申请内存空间。`size`是容器的实际大小，`capacity`是大小的上限。当`vector`满载需要进行扩容时，会完全弃用旧空间，申请更大的新空间，再将元素复制到新空间中。
...
- `erase(it1,it2)`：删除`vector`容器中位于迭代器`[it1,it2)`指定区域内的所有元素，并返回指向被删除区域下一个位置元素的迭代器。`size`会减小，但`capacity`不变。

- `clear()`：删除`vector`容器中所有的元素。`size`变为`0`，但`capacity`不变。
...
`pair`将两个数据组合，它是由结构体实现，元素分别为`first`和`second`。
...
进行比较时，先比较`pair.first`元素的大小，如果相等则继续比较`pair.second`元素的大小：
...
序列式容器`vector`储存的元素都是“值”的概念，关联式容器储存的元素是“键值对”的概念。如果已知目标元素的键值，则直接通过键就可以找到目标元素，不需要遍历整个容器。

关联式容器存储的元素，默认会根据键值做升序排序。STL标准库在实现时底层选用的数据结构是`红黑树`。

### 初始化、访问、遍历

`map`中的元素默认按照键的顺序进行**从小到大**排序
...
- `insert(pair)`：插入一个键值对`pair=(key,value)`。<br>如果成功，返回`(新插入的val的迭代器,true)`；<br>如果失败，说明已经有键为`key`的键值对，返回`(此元素的迭代器,false)`。
...
- `find(key)`：查找键为`key`的元素，返回该元素的迭代器，找不到则返回迭代器`end()`。
...
`count(key)`返回键的数量（这个普通的`map`也可以用）。

```cpp
cout<<mm1.count("a")<<endl; //3
```

`erase(key)`会擦除`key`对应的所有值。
...
使用`set`容器存储的各个键值对，要求键`key`和值`value`必须相等。实际上这可以看作，`set`容器只需要一个`key`信息，即可成功将元素存储起来。同样的，`set`也会对元素进行默认从小到大的排序。
...
- `insert(x)`：插入一个元素`x`。<br>如果成功，返回`(新插入的x的迭代器,true)`；<br>如果失败，说明已经有元素`x`，返回`(x的迭代器,false)`。
...
- `find(x)`：查找元素`x`，返回该元素的迭代器，找不到则返回迭代器`end()`。
...
`find()`、`count()`、`erase()`与`multimap`类似。
...
无序容器也使用“键值对”的方式存储数据。由于不需要维持有序，无序容器的底层实现采用的是`哈希表`。
...
优先队列是一种特殊的队列，它的出队顺序是按照元素的优先级来的。优先队列默认是`大根堆`。
