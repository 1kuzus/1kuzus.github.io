...
需要输入一个`12`位密码，并通过`4`个`check`函数：
...
`check3`会调用`eval_int`函数，所以加一个`print`语句打印出参数，发现`eval`了`password`一些位置的值，以及一个表达式：

- 当输入`password="Hxxxxphxxxxx"`：

    ```text
    Check3 : 120 + 2 == 120 and 120 == 120 and (120 - x) == 120  and 120 > 48 and 120 < 57 and 120 > 48 and 120 < 57 and 120 > 48 and 120 < 57
    ```

- 当输入`password="Hefghphklmno"`：

    ```text
    Check3 : 108 + 2 == 111 and 107 == 108 and (111 - h) == 111  and 107 > 48 and 107 < 57 and 108 > 48 and 108 < 57 and 111 > 48 and 111 < 57
    ```

其中第二个密码的构造使得表达式中`100+i`刚好与`password[i]`位置对应；记`ord(password[i])`为`p[i]`，`check3`的规则为：

- `p[8] + 2 = p[11]`
- `p[7] = p[8]`
- `password[4] = "0"`
- `p[7]`/`p[8]`/`p[11]`的范围为`48`~`57`

`check4`:

`check4`同样会调用`eval_int`函数，可以从打印出的参数得知`check4`是在对密码中剩下的位做校验。

静态分析比较复杂，但是注意到在`check4`的反汇编中，如果检查的某几个位置不符合规则，则函数会提前返回；因此可以根据`eval_int`函数的被调用次数大致判断`check4`的返回点。每次打印出的`eval_int`参数就是当前在检查的某几个位。考虑爆破：

<!-- @xprops highlightLines="3,14-16" -->
...
## Rev/SwiftPasswordManager: ClickMe

GUI程序有一个被禁用的`Flag`按钮，查了一下相关的API为`gtk_widget_set_sensitive`，因此hook这个函数，修改参数，使得控件可交互。

```bash
frida-trace -f ./swiftpasswordmanager -i "gtk_widget_set_sensitive"
```
...
`__handlers__/libgtk_4.so.1.1800.6/gtk_widget_set_sensitive.js`：
...
## Rev/SwiftPasswordManager: LoadMe

保存功能会根据用户输入的一个密码派生出AES加密密钥，然后对数据加密保存；本题需要我们逆向保存功能的逻辑，实现加载功能。附件已经给出了输出数据`passwords.spm`和密码`DUCTF2025!`。

输出数据中主要有随机数`salt`、`nonce`、`tag`和密文，下面的`hook.js`主要hook了三个函数：

- `AES.GCM.Nonce.init()`，可以获得`nonce`值从而得到其在输出数据中的偏移；
- `swift_stdlib_random`，同理这是为了确认`salt`的偏移；
- `SHA256.update(bufferPointer:)`，密钥派生过程。
...
```bash
frida -n swiftpasswordmanager -l hook.js
```
...
大模型提示词注入+工具调用存在SQL注入。[官方WP](https://github.com/DownUnderCTF/Challenges_2025_Public/blob/main/ai/ductfbank-3/solve/WRITEUP.md)思路是在昵称中进行注入，因为大模型有读取昵称的能力（工具），并且[提示词](https://github.com/DownUnderCTF/Challenges_2025_Public/blob/main/ai/ductfbank-1/src/backend/src/constants.ts)中说工具输出结果是可信的。我们直接在聊天中提示词注入了：

#### Q1

验证攻击可行性（有时大模型会编造假的输出结果），获取`sqlite_version()`：
