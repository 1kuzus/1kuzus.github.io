...
export default function Post() {
    return (
        <>
            ...
            <X.P>
                现在我们来考虑更复杂一点的情况，要估计的不是某个概率数值，而是样本分布的概率密度函数。考虑概率密度的性质：概率密度函数在区间$[x,x+\Delta
                x]$下的面积即为样本落在区间$[x,x+\Delta
                x]$内的概率。\n表述的更抽象一点，我们要估样本出现在面积为$V$的一个小区域$R$中的概率，这里的$R$就相当于上面例子中的区间$[x,x+\Delta
                x]$，$V=\Delta x$。
            </X.P>
            <X.P>
            ...
            <X.P>
                `kNN`的全称是`k Nearest
                Neighbors`，意思是K个最近的邻居。基本做法是：根据样本总数确定一个参数$k_n$，例如样本总数为$n$时，可以取
            </X.P>
            ...
            <X.H1>Parzen窗法</X.H1>
            <X.P>
                在这种方法中，我们会定义一个核函数（也称窗函数）$K(x,x_i)$，其含义是，一个观测样本$x_i$对在$x$处概率密度估计的贡献。概率密度估计就是在每一点上把所有观测样本的贡献进行平均，即：
            </X.P>
