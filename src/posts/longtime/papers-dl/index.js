    ...
    return (
        <>
            ...
            <PaperSummary
                ...
                innovation="将输入坐标位置编码，帮助MLP表示高频函数；分层采样"
                limitation="有效地优化和渲染神经辐射场；可解释性"
            />
            <X.HighlightBlock background="blue">
                ...
                <X.Uli>
                    *峰值信噪比*`(Peak Signal to Noise Ratio,
                    PSNR)`：用于衡量图像恢复的质量，数值越高表示图像质量越好。接近`50 dB`代表误差非常小，大于`30
                    dB`人眼难察觉差异。
                </X.Uli>
                <X.Uli>
                    *结构相似性*`(Structural Similarity Index Measure,
                    SSIM)`：用于衡量图像的结构相似性，得分通常在`0`~`1`之间，数值越高表示图像结构越相似。相较于PSNR在图像质量的衡量上更能符合人眼对图像质量的判断。
                </X.Uli>
                <X.Uli>
                    *基于学习的感知图像质量评价*`(Learned Perceptual Image Patch Similarity,
                    LPIPS)`：测量从预训练网络中提取的两个图像的特征之间的相似性，得分通常在`0`~`1`之间，数值越低表示感知质量越高。
                </X.Uli>
            </X.HighlightBlock>
            ...
            <PaperSummary
                topic="主要用于解决NeRF在对全连接神经网络进行参数化时的效率问题。"
                innovation="提出了一种基于哈希搜索的编码方法。"
            />
            <X.HighlightBlock background="blue">
                <X.H3>更多笔记</X.H3>
            ...
            <X.HighlightBlock background="blue">
                ...
                <X.P>
                    ...
                    SFM)`用一组照片估计稀疏点云合成新视图；*多视点立体视觉*`(Multi-View Stereo,
                    MVS)`；\n神经渲染和辐射场：用CNN估计混合权重，用于纹理空间；Soft3D提出`Volumetric
                    representations`；NeRF提出重要性采样和位置编码来提高质量，但使用了大型多层感知器，对速度有负面影响。
                </X.P>
                <X.H3>稀疏重建和稠密重建</X.H3>
                <X.P>
            ...
            <PaperSummary
                ...
                method={
                    <>
                        ...
                        <X.P>
                            NeRF-RCNN用预训练的NeRF提取的辐射场和密度场，为每个检测到的对象输出3D掩码；Mask2Former生成从NeRF渲染的图像的二维全景分割图（跨视图的实例标签并不一定一致）。然后按照相机位置，投影3D掩码去匹配不同视图中的相同实例，去产生多视图一致的2D分割图。CascadePSP用于细化2D
                            mask。
                        </X.P>
                    </>
                }
            ...
            <PaperSummary
                topic="提出R-CNN`(Regions with CNN features)`提高目标检测性能。"
                method="区域提议`(Region Proposals)`：使用`selective search`生成候选。"
            ...
            <X.HighlightBlock background="blue">
                ...
                <X.P>
                    R-CNN的主要性能瓶颈在于，对每个提议区域，卷积神经网络的前向传播是独立的，而没有共享计算。由于这些区域通常有重叠，独立的特征抽取会导致重复的计算。Fast
                    R-CNN的主要改进之一，是仅在整张图象上执行卷积神经网络的前向传播，并且引入*兴趣区域池化*`(ROI
                    Pooling)`，将卷积神经网络的输出和提议区域作为输入，输出连结后的各个提议区域抽取的特征。
                </X.P>
                <X.P>
                    兴趣区域池化层可以给出固定大小的输出：把给定的锚框均匀分割成$n \times
                    m$块，输出每块里的最大值，这样无论锚框多大，总是输出$nm$个值。
                </X.P>
                <X.P>
                ...
                <X.P>
                    ...
                    Proposal Network, RPN)`，模型的其余部分保持不变。区域提议网络作为Faster
                    R-CNN模型的一部分，是和整个模型一起训练得到的。换句话说，Faster
                    R-CNN的目标函数不仅包括目标检测中的类别和边界框预测，还包括区域提议网络中锚框的二元类别和边界框预测。作为端到端训练的结果，区域提议网络能够学习到如何生成高质量的提议区域，从而在减少了从数据中学习的提议区域的数量的情况下，仍保持目标检测的精度。
                </X.P>
            ...
            <PaperSummary
                ...
                method={
                    <>
                        <X.P>
                            基于Faster
                            R-CNN，添加一个对每个ROI预测分割mask的分支。这个分支可以与分类和边界框预测分支并行。
                        </X.P>
            ...
            <X.HighlightBlock background="blue">
                ...
                <X.P>
                    *语义分割*`(semantic
                    segmentation)`：为每一个像素分配一个类别，但不区分同一类别之间的对象。\n*实例分割*`(instance
                    segmentation)`：会区分属于同一类别的不同实例。
                </X.P>
            </X.HighlightBlock>
            ...
            <X.HighlightBlock background="blue">
                ...
                <X.H3>FCN中的转置卷积</X.H3>
                <X.P>
                    例如对于ImageNet的图片输入，大小通常为$224 \times 224 \times
                    3$（RGB通道）；经过卷积后缩小宽高缩小`32`倍，通道增加到`512`，变成$7 \times 7 \times
                    512$的特征图。此时FCN会先通过一个`1x1conv`进行通道降维，然后通过转置卷积将特征图的高度和宽度增加`32`倍，*输出通道数等于类别数*，相当于储存了对每一类的预测结果。
                </X.P>
            </X.HighlightBlock>
            <X.H2 href="https://arxiv.org/pdf/1911.11907">
