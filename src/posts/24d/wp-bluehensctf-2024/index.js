...
export default function Post() {
    return (
        <>
            ...
            <X.P>
                尝试了两种满足条件的解法发现解码的消息都是乱码，因此直接想爆破了；理论上每一个小块可以是`0`/`1`/`2`（不填/黑/红），但是刚试二进制的组合时就成功了。代码如下，直接贴到控制台运行即可。注意要在调试没结束的状态下运行，不然有一些局部变量未定义。
            </X.P>
            <X.CodeBlock
                language="js"
                code={`
            ...
            <X.P>
                用很多俚语/梗代替了正常的C关键字，但是语法结构没有破坏，可以一点一点猜出来，边猜边全局替换，就越看越懂了。题目的大意就是给`flag`设置了一系列的验证规则，需要全部通过。解题的过程就是根据规则确定`flag`的中间位。第`25`位没有线索，是根据源码中的`ohio`推测出来的。
            </X.P>
            <X.CodeBlock
                language="python"
                code={`
            ...
            <X.H2>XS3</X.H2>
            <X.P>
                我做的时候大致的历程是三步：首先，本题是同一个`key`加密多个明文，可以利用ASCII字符之间异或小于`64`，而ASCII字符异或空格大于`64`这个特性，第一阶段先过出可能被泄露的位：
            </X.P>
            <X.CodeBlock
                language="python"
                code={`
            ...
            <X.P>
                剩下的部分中`.`可能是这一位上刚好没有任何密文的对应位置是空格，或者这一位是ASCII码值小于`64`的字符（比如数字）等等。但是先验知识一眼看出`flag`前几位是`udctf`，就可以进入第二阶段的猜测，因为题目说了密文是诗歌中的句子，总可以根据某句话的语义推测出一个没拼写完的单词，只要恢复任意一条密文的下一个位置，就可以恢复`key`的一位。
            </X.P>
            <X.P>
                理论上可以一直重复下去，但其实猜到一定数量就可以Google一下了，找到原文：@[https://milton.host.dartmouth.edu/reading_room/comus/text.shtml]@
            </X.P>
            <X.P>
                第三阶段，找到其中一句（长度够的话）就可以恢复全部到密钥。最后的代码如下，但这并不是一次就写完的，是边猜边补充的。
            </X.P>
            <X.CodeBlock
                language="python"
                code={`
            ...
            <X.P>
                PHP代码中注释的颜色是亮橙色，因此推测`5a4b`是`//`的加密，然后恢复出密钥的前两位；后面的猜测就跟XS3非常相似，例如看到`ec`出现了几次自然就会猜测后两位是`ho`，从而继续恢复密钥。编程语言猜起来比诗歌简单很多。
            </X.P>
            <X.CodeBlock
                language="python"
                code={`
