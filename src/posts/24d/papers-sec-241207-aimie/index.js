...
export default function Post() {
    return (
        <>
            <X.P>相关链接：</X.P>
            <X.Uli>
                @Understanding and Detecting Abused Image Hosting Modules as Malicious
                Services[https://www.xiaojingliao.com/uploads/9/7/0/2/97024238/hong2023understanding.pdf]@
            </X.Uli>
            ...
            <X.P>
                论文把其研究的这种现象命名为AIMIEs`(*A*bused *I*HMs as *M*al*I*cious
                s*E*rvices)`，这种服务可以是开源的（比如@AUXPI[https://github.com/0xDkd/auxpi]@），也可以是商用的（出于盈利目的）。
            </X.P>
            <X.P>论文的主要贡献是：</X.P>
            ...
            <X.H2>IHM</X.H2>
            <X.P>
                论文提到的IHM`(Image Hosting
                Module)`可以理解为“私有图床”，这里格外强调了IHM主要是服务于网站的特定业务功能内部使用的，如前面提到的问答平台、聊天功能等，IHM并非旨在作为一个独立图像托管平台运行。
            </X.P>
            ...
            <X.Uli>
                Abused upload API
                detection：研究团队开发了一个多语言解释器，先将收集到的AIMIE源码生成AST，然后解析字符串相关操作（拼接、格式化函数等），然后提取并验证出现的字符串值是否为一个URL（比如匹配以`http:`或`https:`开头），然后确定这是否是一个被利用的IHM上传图片的API（如果该API位于一个未被设计用于图片上传、但具有图片上传功能的网站上）。
            </X.Uli>
            <X.Uli>
                Abused hosting domain
                detection：研究团队部署了其中`14`个开源的AIMIE，触发其中的上传API，并抓包响应流量以确定存储服务器的域名。
            </X.Uli>
            ...
            <X.Uli>如果需要前端预览，可以使用客户端缓存；</X.Uli>
