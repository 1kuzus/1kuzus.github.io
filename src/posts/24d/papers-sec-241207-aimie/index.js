...
export default function Post() {
    return (
        <>
            ...
            <X.P>工具中使用Puppeteer爬虫模拟用户的使用过程，前两个阶段上传图像，后两个阶段填写表单内容并模拟点击上传按钮。对于表单内容工具自定义了一组输入，有对应`type`的`input`元素就自动填入。在测试的全程监听页面的`request`和`response`事件，并对请求和响应中出现的可能是图片的URL进行正则提取。</X.P>
            <X.P>再然后，工具中利用MD5和@blockhash-js（图像感知哈希）[https://www.npmjs.com/package/blockhash]@去验证提取的URL是否是上传的图像（MD5一致），或者非常相似（感知哈希值海明距离小于阈值）。</X.P>
