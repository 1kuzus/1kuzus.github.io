...
export default function Post() {
    return (
        <>
            ...
            <X.P>
                传统神经网络每次的输入是独立的，每次输出只依赖于当前的输入；但在某些任务中需要更好的处理序列信息，即前面的输入和后面的输入是有关系的；*循环神经网络*`(Recurrent
                Neural Networks, RNN)`通过使用带自反馈的神经元，能够处理任意长度的序列。
            </X.P>
            <X.P>
            ...
            <X.Oli reset>
                <X.P>
                    定义模型时，不再需要显式指定`linear_ih`和`linear_hh`两层，将由`nn.RNN`模块实现；`nn.RNN`模块没有定义输出层，因此输出层`linear_ho`需要设置。\n在`forward`函数中，手动实现时为了直观展示出RNN的迭代过程，只进行了一次隐藏状态的更新；而对于输入序列迭代更新隐藏状态是在训练和预测时实现的。而`nn.RNN`模块的一次`forward`就已经完成了迭代更新，其输入是整个序列`seq`和`prev_state`，返回值是`output_hidden,curr_state`，对于不考虑批量大小的数据，它们的`size`为：
                </X.P>
            ...
            <X.P>
                ...
                ，我可以说流利的【法语】`，这个情况下，随着前后文距离变大，RNN对长期依赖关系的学习会变得困难。
            </X.P>
            <X.H2>LSTM</X.H2>
            <X.P>
                *长短期记忆网络*`(Long Short-Term Memory,
                LSTM)`是一种特殊的RNN，可以学习长期依赖。以RNN为例，循环神经网络随时间展开通常具有如下的示意图：
            </X.P>
            ...
            <X.HighlightBlock background="gray">
                ...
                <X.P>
                    当模型看到`Alice是一名女教师，……`时，单元状态中可能存储了和主语`Alice`和`女教师`有关的语义信息，以便在后文输出合适的代词`她`；然后，当模型看到`Alice是一名女教师，她喜欢给学生讲课；Bob是一位男司机，……`时，我们希望在看到新主语`Bob`和`男司机`之后，忘记此前存储的旧主语的性别语义。也就是对旧单元状态
                    {`$C_{t-1}$`}乘上较小的$f_t$。
                </X.P>
            </X.HighlightBlock>
            <X.H3>输入门</X.H3>
