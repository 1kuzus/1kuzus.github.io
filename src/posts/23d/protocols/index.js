...
export default function Post() {
    return (
        <>
            ...
            <X.P>
                *停等协议*`(Stop and Wait,
                SW)`的发送方每发送一帧数据就停止，并等待接收方发送确认帧，收到确认后再发送下一帧。
            </X.P>
            ...
            <X.H3>确认迟到</X.H3>
            <X.P>
                如果确认分组迟到，在图示情况中，发送方需要感知到第二个`ACK`是对第二个`DATA 0`的重复确认，而不是对`DATA
                1`的确认。因此确认分组也需要使用一位编号。
            </X.P>
            <X.H2>回退N帧协议 - GBN</X.H2>
            <X.P>
                *回退N帧协议*`(Go Back N,
                GBN)`允许发送方连续发送多个帧（即发送窗口可以大于`1`），以解决停等协议信道利用率低的问题。
            </X.P>
            <X.P>
            ...
            <X.P>
                接收方不一定要对收到的数据逐个确认，也可以对最后一个分组进行确认。`ACK
                n`表示序号为`n`及其之前的分组都已经被正确接收。称这种方法为累计确认。例如图中，接收方只对`1`号、`4`号数据分组进行确认，并且`ACK
                1`在传输过程中丢失，发送方仍然可以根据收到的`ACK 4`判断`0`~`4`号分组都已经被正确接收。
            </X.P>
            <X.H3>有差错情况</X.H3>
            <X.Image src="gbn2.png" width="100%" filterDarkTheme />
            <X.P>
                例如发送方发送了`5`/`6`/`7`/`0`/`1`五个分组，但`5`号数据分组丢失，此时由于到达的四个分组都与接收窗口序号不匹配，接收方会将其丢弃，并重复响应`ACK
                4`。
            </X.P>
            <X.H2>选择重传协议 - SR</X.H2>
            <X.P>
                *选择重传协议*`(Selective Repeat,
                SR)`加大接收窗口的长度（即接收窗口也可以大于`1`），缓存乱序到达的帧，这样不至于在一些情况下需要重传所有未被确认的帧从而导致效率降低。
            </X.P>
            <X.P>
            ...
            <X.P>
                如果`4`号、`5`号可以被正常接收，并且返回的`ACK 4`、`ACK
                5`可以正确到达发送方，那么最后会使得发送窗口和接收窗口都卡在`2`的位置，并最终触发`2`号分组超时重传。
            </X.P>
            <X.HighlightBlock>
            ...
            <X.P>
                *载波监听多路访问*`(Carrier Sense Multiple Access,
                CSMA)`的思想是在发送帧之前监听信道。其原理是：当几个站同时在总线上发送数据时，总线上的信号电压摆动值将会互相叠加而增大。当一个站检测到的信号电压摆动值超过一定门限值时，就认为总线上至少有两个站同时在发送数据，表明产生了碰撞。
            </X.P>
            <X.H3>非坚持CSMA</X.H3>
            <X.P>
            ...
            <X.P>
                上述的三种CSMA发生冲突后，仍然会坚持把数据帧发完，这时接收方收到叠加在一起的有碰撞数据会响应否认`NAK`或者不响应等待发送方重发，这是一种被动的方式。*碰撞检测*`(Collision
                Detection,
                CD)`就是边发送数据边检测信道上信号电压的变化情况，来判断是否有其他主机也在发送数据。如果发现碰撞则立刻主动终止发送。
            </X.P>
            <X.H3>强化碰撞</X.H3>
            <X.P>
                以太网的强化碰撞：当发送帧的站点检测到碰撞，除了立即停止发送帧外，还要再继续发送`48`位的人为干扰信号，以便有足够多的碰撞信号使所有站点都能检测出碰撞。
            </X.P>
            <X.H3>争用期</X.H3>
            <X.Image src="csma1.png" width="100%" filterDarkTheme />
            <X.P>
                图中$\tau$是单程传播时延，$\delta$并没有很强的物理含义，我们只是假设主机D发送数据后经过$\delta$时间检测到A的信号。这张图的重点是，在$\delta$趋近于0的情况下，A最长需要$2\tau$的时间检测到碰撞。我们把$2\tau$称作*争用期*`(Contention
                Period)`，它的意义是只要经过$2\tau$时间还没有检测到碰撞，就能肯定这次发送不会发生碰撞。
            </X.P>
            ...
            <X.P>
                如果一个帧的长度太小，可能一个站点还没来得及检测到其发送的帧产生了碰撞，就已经将这个帧发送完毕了。帧的传输时间要大于争用期。对于`10
                Mb/s`以太网，帧的最小长度为`64`字节。减去帧头和帧尾，数据部分至少应为`46`字节。
            </X.P>
            <X.P>
            ...
            <X.P>
                在无线局域网中可能会出现隐蔽站的问题，使用碰撞检测意义不大。不同于以太网，802.11标准使用*碰撞避免*`(Collision
                Avoidance, CA)`而不是碰撞检测。
            </X.P>
            ...
            <X.P>
                ...
                RTS)`，包括源地址、目的地址和这次通信所需的时间；\n若目的站收到源站发来的`RTS`帧且媒体空闲，就响应*允许发送*`(Clear
                To Send,
                CTS)`它也包括这次通信所需的时间（从`RTS`复制）。`CTS`帧可以被目的站范围内所有站点收到，其作用有两点：给源站明确的发送许可，同时指示其他站在预约期内不要发送。
            </X.P>
            <X.H1>网络层</X.H1>
            ...
            <X.P>
                ...
                protocol)`和*自治系统间路由选择协议*`(inter-AS routing
                protocol)`。有时也会把这两个概念称作*内部网关协议*`(Interior Gateway Protocol,
                IGP)`和*外部网关协议*`(Exterior Gateway Protocol, EGP)`。
            </X.P>
            ...
            <X.P>
                ...
                Resolution Protocol, ARP)`用于完成网络层地址（例如IP地址）和链路层地址（即MAC地址）的转换。
            </X.P>
            <X.H2>互联网控制报文协议 - ICMP</X.H2>
            <X.P>
                *互联网控制报文协议*`(Internet Control Message Protocol,
                ICMP)`被主机和路由器用来彼此沟通网络层的信息，ICMP最典型的用途是差错报告，提供可能发生在通信环境中的各种问题反馈。通过这些信息，管理者可以对所发生的问题作出诊断，然后采取适当的措施解决。
            </X.P>
            <X.H2>动态主机配置协议 - DHCP</X.H2>
            <X.P>
                *动态主机配置协议*`(Dynamic Host Configuration Protocol,
                DHCP)`是使用UDP协议的应用层协议，用于自动给内网机器分配IP地址等信息。虽然可以手动给内网设备一个一个的分配IP地址，但这项任务通常由DHCP服务器完成。
            </X.P>
            <X.P>DHCP的四个步骤是：</X.P>
            <X.Uli>
                *DHCP发现*`(DHCP
            ...
            <X.P>
                *用户数据报协议*`(User Datagram Protocol,
                UDP)`提供无连接、不可靠的服务，支持单播、多播、广播；UDP面向报文，对应用进程交下来的报文既不合并也不拆分。
            </X.P>
            <X.P>
            ...
            <X.P>
                *传输控制协议*`(Transmission Control Protocol,
                TCP)`提供连接、可靠的服务，仅支持单播；TCP面向字节流，把应用进程交下来的报文仅当作字节流。
            </X.P>
            <X.P>TCP首部由`20`字节固定首部和最大`40`字节的扩展首部构成。下面是对一些字段的解释：</X.P>
            <X.Uli>
                序号、确认号、`ACK`标志位（在Flags中）：用于实现TCP可靠传输；确认号指出期望收到对方下一个TCP报文段的数据载荷的第一个字节的序号，同时也是对之前收到的所有数据的确认。只有当`ACK`取值为`1`时，确认号字段才有效。TCP规定在连接建立后所有传送的报文段都必须把`ACK`置`1`。
            </X.Uli>
            <X.Uli>
                数据偏移：占`4`位，以`4`字节为单位，用来指出TCP报文段的数据载荷部分的起始处距离TCP报文段的起始处有多远；这个字段实际上指出了TCP报文段的首部长度。
            </X.Uli>
            ...
            <X.H3>TCP连接释放</X.H3>
            <X.P>
                TCP通过四报文挥手释放连接。服务端响应客户端的释放请求后进入关闭等待状态，但如果TCP服务器进程还有数据要发送，服务端仍然会发送完最后的数据才会发送连接释放报文段。\n客户端确认了连接释放报文段后，还要经过`2`倍*最长报文段寿命*`(Maximum
                Segment Lifetime,
                MSL)`才进入关闭状态，这是为了防止客户端确认报文丢失，如果直接进入关闭状态就无法接收到服务端重传的连接释放报文。过程如下图所示：
            </X.P>
            ...
            <X.P>
                ...
                n`表示序号为`n`及其之前的分组都已经被正确接收，而TCP中`ACK
                n`表示序号为`n-1`及其之前的分组都已经被正确接收，接收方期望收到的最小序号的分组是序号`n`。
            </X.P>
            <X.P>
                如果接收方收到了失序到达的分组，会缓存这些分组（像SR一样），但不是对分组逐一确认，而是累计确认期望收到的最小序号的分组（类似GBN）。TCP同样只有一个超时计时器（像GBN一样），如果发生超时就会重传窗口内所有已发送但未得到确认的数据。
            </X.P>
            <X.P>
                不过，例如当发送方陆续发送序号`n`之后的分组，但却收到了重复的`ACK
                n`时，就可以判断分组`n`一定因为某些原因没有被接收方收到。
            ...
            <X.HighlightBlock>
                ...
                <X.P>
                    流量控制中接收方告知发送方的窗口大小称为*接收窗口*`(Receiver Window,
                    rwnd)`，其反映了接收方的容量；\n拥塞控制中发送方根据网络拥塞程度调整的发送窗口大小称为*拥塞窗口*`(Congestion
                    Window, cwnd)`，其反映了当前网络容量。
                </X.P>
