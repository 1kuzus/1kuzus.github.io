...
export default function Post() {
    return (
        <>
            ...
            <X.HighlightBlock>
                <X.P>
                    此处有一个历史缘故。在2023年1月的时候，`ul`和`pl`是明文的用户名和密码的字符串长度，这也符合参数名`username
                    length(ul)`，`rsa`是前端加密参数。后来华科登录接口似乎经历了一次改版，弃置了`rsa`参数，`ul`和`pl`变成了现在的意义。
                </X.P>
            </X.HighlightBlock>
            <X.P>
                言归正传，在没有预先知道参数含义的情况下，先从看起来是明文形式的后两个参数入手，直接搜索`_eventId`，找到了HTML中写表单的隐藏域（也就是`input
                type="hidden"`）的位置：
            </X.P>
