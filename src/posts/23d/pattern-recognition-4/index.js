...
export default function Post() {
    return (
        <>
            ...
            <X.H2>常数问题</X.H2>
            <X.P>
                你可能看到过{`$y=\\bm{W}^T\\bm{X}+b$`}和{`$y=\\bm{W}^T\\bm{X}$`}
                两种计算分类结果的方式。以二维空间上的分类为例，前者的分类线可以是任意直线，而后者只能是过原点的直线。事实上，可以通过将问题空间升一个维度来统一两种形式。
            </X.P>
            <X.P>
            ...
            <X.P>
                假设`Sigmoid`函数的输出是$p$，其含义应为“样本是正类”的概率。因此一个正样本对似然函数的贡献是$p$，而负样本要代入$1-p$。换句话说，上述对数似然函数中的
                {`$P(y_i|\\bm{X}_i)$`}可以有如下数学表示：
            </X.P>
            <X.Formula
                text="P(y_i|\bm{X}_i)=
                \begin{cases}
