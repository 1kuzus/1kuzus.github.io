...
export default function Post() {
    return (
        <>
            ...
            <X.P>
                接下来围绕下面的例子逐步介绍`mAP`的计算。在下面的`7`张图片中共有`15`个目标，用绿色框标出；并且有`24`个检测框，用红色框标出，百分比数值表示置信度。
            </X.P>
            ...
            <X.H1>查准率和查全率</X.H1>
            <X.P>
                对于一组图像的目标检测结果来说，假如规定`IoU`大于某一阈值就把检测结果视为真正例`TP`，否则视为假正例`FP`；并且如果出现多个检测重叠一个真值的情况，只有最高置信度的检测框被视为`TP`，其他则视为`FP`。
            </X.P>
            <X.P>
                接下来计算两个指标：*查准率*或*精确率*`Precision`，即预测为正的样本中的正确率`TP/All
                Detections`；*查全率*或*召回率*`Recall`，即正确检测出的正样本占所有正样本的比例`TP/All Ground Truths`。
            </X.P>
            ...
            <X.P>
                通常的计算方法是取曲线`11`点插值，不过无论如何计算，最后的含义都是希望得到近似的曲线下面积。因此在这里不展开介绍计算过程，而是讨论“曲线下面积”是如何与“平均精度”建立起联系的。
            </X.P>
            <X.HighlightBlock background="gray">
                <X.H3>个人理解</X.H3>
                <X.P>
                    当我们说模型在一个类别上预测精度是`80%`时，直观上的体会是“模型的`10`个预测框中，大约有`8`个是对目标的正确预测”。然而这并不能与`Precision`-`Recall`曲线下的面积建立起直观的联系。但注意到曲线的纵坐标的含义就是精度，从这个角度出发，以下是对`AP`指标的个人理解：
                </X.P>
                <X.P>
            ...
            <X.P>
                评估模型在目标检测任务上的性能时，通常用所有类别的`AP`的平均值，而不是将所有类别的检测结果混在一起，直接计算出一个数值，其中一个原因是：\n当类别数量不平衡时，可能某个少样本类别的检测效果很差。如果混在一起计算，由于该类别样本数量较少，因此并不会对最终的结果产生很大的影响。此时对模型的性能评估是不准确的。因此需要逐类测量`AP`再取均值，作为最终的`mAP`检测结果。
            </X.P>
            <X.H1>参考资料</X.H1>
