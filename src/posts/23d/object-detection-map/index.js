...
export default function Post() {
    return (
        <>
            ...
            <X.P>接下来围绕下面的例子逐步介绍`mAP`的计算。在下面的`7`张图片中共有`15`个目标，用绿色框标出；并且有`24`个检测框，用红色框标出，百分比数值表示置信度。</X.P>
            <X.Image src="fig1.png" width="600px" />
            <X.H1>交并比</X.H1>
            <X.P>*交并比*`(Intersection over Union, IoU)`是产生的检测框与真实检测框的交集与并集的比值。</X.P>
            <X.Image src="fig2.jpg" width="300px" themeAdaptive />
            <X.H1>查准率和查全率</X.H1>
            ...
            <X.Image src="fig3.png" width="80%" />
            <X.P>表格的前三列分别是图片编号、检测框编号和置信度；\n`TP``FP`列标识出这个检测框是否为一个真正例；\n`Acc TP``Acc FP`列是截至此行累计的`TP`和`FP`个数，用于`Precision`和`Recall`的计算；\n`Precision`列由`Acc TP/(Acc TP + Acc FP)`得到；\n`Recall`列由`Acc TP/15`得到（共有`15`个目标，因此`All Ground Truths`=`15`）。</X.P>
            <X.H1>平均精度AP</X.H1>
            ...
            <X.HighlightBlock background="gray">
                ...
                <X.Image src="fig4.jpg" width="40%" themeAdaptive />
                ...
                <X.Image src="fig5.jpg" width="85%" themeAdaptive />
            ...
            <X.P>评估模型在目标检测任务上的性能时，通常用所有类别的`AP`的平均值，而不是将所有类别的检测结果混在一起，直接计算出一个数值，其中一个原因是：\n当类别数量不平衡时，可能某个少样本类别的检测效果很差。如果混在一起计算，由于该类别样本数量较少，因此并不会对最终的结果产生很大的影响。此时对模型的性能评估是不准确的。因此需要逐类测量`AP`再取均值，作为最终的`mAP`检测结果。</X.P>
            <X.H1>参考资料</X.H1>
            ...
            <X.Uli>@深入了解平均精度(mAP)：通过精确率-召回率曲线评估目标检测性能[https://baijiahao.baidu.com/s?id=1767101899839497831]@</X.Uli>
        </>
    );
}
