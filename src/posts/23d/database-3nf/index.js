...
export default function Post() {
    return (
        <>
            <X.P>
                为了减少数据冗余，设计数据表时必须遵循一定的规则，在关系型数据库中这种规则就称为范式。\n严谨的定义可以参考维基百科@第一范式[https://zh.wikipedia.org/zh-cn/%E7%AC%AC%E4%B8%80%E6%AD%A3%E8%A6%8F%E5%8C%96]@、@第二范式[https://zh.wikipedia.org/zh-cn/%E7%AC%AC%E4%BA%8C%E6%AD%A3%E8%A6%8F%E5%8C%96]@、@第三范式[https://zh.wikipedia.org/zh-cn/%E7%AC%AC%E4%B8%89%E6%AD%A3%E8%A6%8F%E5%8C%96]@，互联网上也有很多优秀的技术博客。本文希望以简洁的语句和例子进行描述。
            </X.P>
            <X.H1>第一范式 - 1NF</X.H1>
            <X.HighlightBlock>
            ...
            />
            <X.P>
                这个例子比上一个更好吗？哪一种设计更符合第一范式？事实上，两个例子都是符合的，具体的取舍要参考具体的业务。例如`姓名`、`地址`在业务中都整体出现，那么第二个例子就是最佳选择。如果业务涉及*分别*对所属省、市的操作，那么就应该将`地址`字段拆分为`省`和`市`。
            </X.P>
            <X.H1>第二范式 - 2NF</X.H1>
            <X.HighlightBlock>
                <X.P>
            ...
            />
            <X.P>
                这个例子中，`成绩`是针对某一学生的某一课程而言的，因此需要`学号`和`课程代码`作为联合主键才能确定；然而学生的`学生姓名`仅由`学号`就可以唯一确定，这就叫部分依赖于主键，因此不符合第二范式。正确的做法是将`学生姓名`和`成绩`分开存放于两张表中：
            </X.P>
            <X.FlexRow gap="32px">
                <X.Table
                    fromText={`
            ...
            />
            <X.P>
                这个例子不符合第三范式，因为存在间接的传递决定关系：`id → 城市 →
                城市人口`。`城市人口`字段直接依赖的是`城市`字段，而不是主键`id`。正确的做法是设计两张表：
            </X.P>
            <X.FlexRow gap="32px">
                <X.Table
                    fromText={`
