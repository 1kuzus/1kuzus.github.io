...
export default function Post() {
    return (
        <>
            <X.H1>模型量化技术</X.H1>
            <X.P>
                模型量化的主要思路是寻找一个映射函数，将高精度的值映射为低精度的值（降低位宽），且保证效果相似，以降低存储成本和计算需求。量化的对象通常是权重数据和激活数据。根据量化间距是否相等分为*线性量化*（均匀量化）和*非线性量化*（非均匀量化）。
            </X.P>
            <X.H2>线性量化</X.H2>
            <X.H3>分类</X.H3>
            ...
            <X.H2>网络剪枝</X.H2>
            <X.P>
                给定一神经网络模型，去除该模型中对最终结果影响不大的参数，获得结构更为精简的模型，该过程被称为网络剪枝`(Neural
                Network Pruning)`。
            </X.P>
            <X.H3>非结构化剪枝</X.H3>
            <X.P>
            ...
            <X.H3>常规卷积</X.H3>
            <X.P>
                假设卷积核$h \times w$，输入通道$D_i$，输出通道$D_o$，常规卷积需要的参数量为$h \times w \times D_i
                \times D_o$。
            </X.P>
            <X.H3>分组卷积</X.H3>
            <X.Image src="fig3.jpg" width="600px" filterDarkTheme />
            <X.P>
                如果分成$g$组，可以理解为把整个卷积考虑为$g$个子问题，每个子问题是：\n求解$H_i \times W_i \times
                (D_i/g)$输入数据，经过卷积得到$H_o \times W_o \times
                (D_o/g)$输出数据。\n最后将得到的$g$个输出数据拼接起来，得到$H_o \times W_o \times D_o$输出。
            </X.P>
            <X.P>
                在这个过程中，每一组卷积核大小为$h \times w \times
                (D_i/g)$，卷积核的个数为$(D_o/g)$，因此每组卷积的参数量为$h \times w \times (D_i/g) \times
                (D_o/g)$。由于一共有$g$组，所以总参数量为$h \times w \times D_i \times D_o \times 1/g$。
            </X.P>
            ...
            <X.P>
                深度可分离卷积是深度卷积和逐点卷积的组合，首先对输入数据进行深度卷积，$g=D_i$，参数量为$h \times w
                \times D_i$；然后对中间输出数据进行逐点卷积使得通道数变为$D_o$，这一步的参数量为$D_i \times D_o$。
            </X.P>
