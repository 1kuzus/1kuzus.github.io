...
export default function Post() {
    return (
        <>
            <X.P>相关链接：</X.P>
            <X.Uli>
                @A Systematic Study of the Consistency of Two-Factor Authentication User Journeys on Top-Ranked
                Websites[https://www.ndss-symposium.org/wp-content/uploads/2023/02/ndss2023_s362_paper.pdf]@
            </X.Uli>
            ...
            <X.HighlightBlock background="blue">
                ...
                <X.P>
                    @用户旅程[https://en.wikipedia.org/wiki/User_journey]@是用户与某物（通常是软件）交互时的体验，或为实现某个目的需要经历的操作过程。
                </X.P>
                <X.H3>雅各布定律</X.H3>
                <X.P>
            ...
            <X.P>
                本文系统地研究了`85`个热门网站上的2FA用户旅程，也就是希望确定这些网站是否始终遵循相同的设计模式和策略。为了做更准确的比较，论文设计了一个包含`22`个比较因子的列表，然后比较得出一般的设计模式，也给出对UX设计有益/有害的模式。
            </X.P>
            ...
            <X.P>
                UX设计中的*雅各布定律*表示了功能类似的产品的设计，应当具有尽可能一致的模式，这样才能降低用户学习的成本，这也是本文研究各个网站2FA用户旅程的一致性的核心动机。
            </X.P>
            ...
            <X.P>
                本文工作的关键区别在于，本文不研究形式、内容或功能的具体变化如何影响2FA的易用性和具体体验，而是首次系统地研究现有流行网站用户体验的*一致性*。论文的核心贡献是衡量各个网站的2FA用户旅程服从雅各布定律的程度。
            </X.P>
            ...
            <X.Table
                ...
                Grouped-Setting. 集中的设置位置（“多选”至少为'部分满足'）|用户有一个单一的设置位置来管理他们所有的2FA选项|/|和2FA相关的设置项较为分散
                No-Enforced-Options. 无强制选项（“多选”至少为'部分满足'）|用户可以自主选择2FA选项|/|例如，用户必须先配置短信验证，然后才能配置其他选项
                Selectable-Primary-Option. 可以设置主选项（“多选”应为'满足'）|可以设置主选项|/|不支持用户自主选择的主选项
            ...
            <X.Table
                ...
                Deactivation-Verification. 停用需认证（“可停用”至少为'部分满足'）|停用2FA时用户需要身份认证|/|停用2FA时用户不需要身份认证
            ...
            <X.P>一些发现包括：</X.P>
            <X.Uli>
                `86%`的网站使用`two-factor`/`two-step`/`multiple-factor`+`authentication`/`verification`作为命名；
            </X.Uli>
            <X.Uli>`92%`的网站2FA设置在`账户设置`的`安全设置`下；</X.Uli>
            <X.Uli>
                只有`31`个网站提供设备记忆功能，其中`52%`的网站描述设备记忆为`记住设备或客户端`，`29%`的网站描述设备记忆为`信任设备`，`13%`的网站描述设备记忆为`跳过额外步骤`；
            </X.Uli>
            <X.Uli>……</X.Uli>
            <X.H2>探索性数据分析</X.H2>
            <X.Uli>
                网站相似性和因素一致性：使用汉明距离分析，发现平均每个网站在`14`个非条件性因素中有`6`~`7`个因素与其他网站不同，表明这些网站的2FA用户流程在整体上缺乏一致性。
            </X.Uli>
            <X.Uli>
                通过聚类分析，研究发现`6`个主要的设计模式（聚类集群），这些集群在用户教育、多重2FA选项的支持以及设备记忆功能的提供上有所不同。
            </X.Uli>
            <X.Uli>
