...
export default function Post() {
    return (
        <>
            ...
            <X.H3>assume</X.H3>
            <X.P>
                `assume`伪指令用来指定段寄存器和段名之间的关系。在这个例子中，`assume
                cs:codesg`表示`CS`寄存器与`codesg`段相关联，将定义的`codesg`当作程序的代码段使用。
            </X.P>
            <X.H1>运行一个汇编程序</X.H1>
            ...
            <X.H2>[...]和(...)</X.H2>
            <X.P>
                `[...]`是汇编语法，表示一个内存单元，段地址在`DS`中，偏移地址由`...`给出；\n`(...)`是为了学习方便做出的约定，表示一个内存单元或寄存器中的内容。
            </X.P>
            ...
            <X.H1>段前缀</X.H1>
            <X.P>
                在`debug`中使用`-a`编写汇编代码访问内存时，可以直接使用`mov
                ax,[idata]`，但在汇编程序中，需要使用段前缀，写为`mov ax,ds:[idata]`。例如在汇编程序中，`mov
                ax,[7]`等同于`mov ax,7`，如果是想访问`ds:7`则需要改写为`mov ax,ds:[7]`。
            </X.P>
