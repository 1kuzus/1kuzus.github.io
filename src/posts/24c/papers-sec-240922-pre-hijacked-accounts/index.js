...
export default function Post() {
    return (
        <>
            ...
            <X.P>像是前两种的结合：首先用目标用户的邮箱地址创建账号，然后与攻击者的IdP账号关联；当用户重置密码时攻击者仍然可以通过联合身份登录。</X.P>
            ...
            <X.P>像是第一种攻击的镜像版本：攻击者通过联合方式注册（在攻击者的IdP上使用目标用户的邮箱），而用户通过普通方式注册，从而导致用户无感知的账户合并。</X.P>
        </>
    );
}
