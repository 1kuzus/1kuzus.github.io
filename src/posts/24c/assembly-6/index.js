...
export default function Post() {
    return (
        <>
            ...
            <X.H1>操作显存</X.H1>
            <X.P>屏幕上的内容其实就是显存中的数据。显存是一块内存区域，在8086 CPU中，显存占用地址`a0000h`~`bffffh`共`128KB`的地址空间，而显示字符时常用到其中的一小块区域：`b8000h`~`bffffh`共`32KB`的地址空间，它是`25`行、`80`列彩色字符模式的显示缓冲区。</X.P>
