...
export default function Post() {
    return (
        <>
            ...
            />
            <X.P>
                和前面学习的地址标号相比，地址标号仅仅表示一个地址，而数据标号表示地址的同时还暗含了数据的类型，操作时不再需要用`byte
                ptr`等显式地指定数据的类型。
            </X.P>
            <X.H1>操作显存</X.H1>
            <X.P>
                屏幕上的内容其实就是显存中的数据。显存是一块内存区域，在8086
                CPU中，显存占用地址`a0000h`~`bffffh`共`128KB`的地址空间，而显示字符时常用到其中的一小块区域：`b8000h`~`bffffh`共`32KB`的地址空间，它是`25`行、`80`列彩色字符模式第`0`页的显示缓冲区。
            </X.P>
            <X.P>
            ...
            <X.P>
                在8086
                CPU中，中断可以打断当前程序的执行，当CPU收到中断请求时，会执行中断处理程序，处理完后再返回到原程序继续执行。中断处理程序的入口地址存放在*中断向量表*中，根据中断类型码查表得到对应处理程序的入口地址。
            </X.P>
            <X.P>
                8086
                CPU的中断向量表是一个`1KB`的表，包含了`256`个中断类型码对应的中断处理程序的入口地址，每个入口地址占`4`个字节，前两字节存放`IP`的值，后两字节存放`CS`的值。中断向量表的起始地址是`00000h`，终止地址是`003ffh`。
            </X.P>
            <X.CodeBlock
                language="text"
                code={`
            ...
            <X.Uli>
                可屏蔽中断：CPU可以不响应的外中断，一般是由外部硬件通过INTR`(Interrupt
                Request)`信号线发送给CPU；CPU是否响应取决于`IF`标志位，如果`IF=0`，CPU不响应可屏蔽中断。
            </X.Uli>
            <X.Uli>
                不可屏蔽中断：CPU必须响应的外中断，通过NMI`(Non-Maskable Interrupt)`信号线发送。8086
                CPU不可屏蔽中断的中断类型码固定为`2`。
            </X.Uli>
            <X.P>
            ...
            <X.H2>单步中断</X.H2>
            <X.P>
                标志寄存器中有两个标志位与中断有关：`TF(Trap Flag)`和`IF(Interrupt
                Flag)`。`TF`用于单步执行，`IF`用于可屏蔽中断的开关。
            </X.P>
            ...
            <X.P>
                在执行`int n`时，逻辑上相当于自动依次执行了：`pushf`、`push cs`、`push
                ip`；它和`call`指令保存`CS`和`IP`的行为很像，但还保存并修改了标志寄存器的值。
            </X.P>
            <X.P>
                对应地，从一个中断处理程序返回到原程序时，需要使用`iret`指令，逻辑上相当于自动依次执行了：`pop ip`、`pop
                cs`、`popf`。
            </X.P>
            <X.H2>int指令示例</X.H2>
            <X.P>
                在这里我们编写设计一个`int
                7ch`中断，功能是将以`0`结尾的纯字母字符串转为大写，参数是`DS:SI`指向字符串首地址。
            </X.P>
            ...
            <X.P>
                现在希望编写一个`i7ch`中断处理程序，由`int
                7ch`触发中断；此时需要一个确定、但又不影响系统的内存位置存放程序；一个技巧是使用中断向量表的内存区域，因为别的程序不会用到，而系统要处理的中断事件也远没有`256`个，因此可以利用中断向量表后段地址空间，这里选取从`00200h`开始的地址空间作为存放`i7ch`程序的目的地址。
            </X.P>
            <X.CodeBlock
                language="asm8086"
            ...
            <X.P>
                执行的效果是相同的：数据段中的字符串会被改写为大写，但要注意此时中断处理程序已经写入了内存，在其他程序中可以直接调用`int
                7ch`实现同样的功能！例如编写一个`test`程序：
            </X.P>
            <X.CodeBlock
                language="asm8086"
                code={`
            ...
            <X.Oli>
                <X.P>引发`int 9`中断</X.P>
                <X.P>
                    键盘的输入到达`60h`端口时，相关的芯片就会向CPU发出中断类型码为`9`的可屏蔽中断信息。CPU检测到该中断信息后，如果`IF=1`，则响应中断，转去执行`int
                    9`中断处理程序。
                </X.P>
            </X.Oli>
            <X.Oli>
                ...
                <X.HighlightBlock>
                    <X.H3>BIOS键盘缓冲区</X.H3>
                    <X.P>
                        BIOS键盘缓冲区是系统启动后，BIOS用于存放`int
                        9`中断处理程序所接收的键盘输入的内存区。BIOS键盘缓冲区可以存储最多`15`个键盘输入，每个键盘输入用一个字存放，高位字节存放扫描码，低位字节存放ASCII码。
                    </X.P>
                    <X.P>
            ...
            <X.P>
                `int 16h`中断和`int 9`中断是一对相互配合的程序，`int 9`中断处理程序向键盘缓冲区中写入，`int
                16h`中断处理程序从缓冲区中读出。它们写入和读出的时机不同，`int
                9`中断处理程序在有键按下的时候向键盘缓冲区中写入数据；而`int
                16h`中断处理程序是在应用程序对其进行调用的时候，将数据从键盘缓冲区中读出。
            </X.P>
            <X.H1>练习</X.H1>
            ...
            <X.P>
                我们现在要在`i9`子程序中调用原来的`int
                9`中断处理程序。请注意，程序之所以跳转到`i9`，是因为程序已经修改了中断向量表，然后由键盘输入产生`int
                9`中断，中断过程的第`1`~`5`步已经执行完成，在第`5`步进入到`i9`子程序中；
            </X.P>
            <X.P>
                而我们还想在`i9`子程序中手动调用被保存在`dword ptr ds:[0]`中的原来的`int
                9`中断处理程序，此时没有人为我们自动执行中断过程了，需要手动模拟！因此这部分过程为：
            </X.P>
            <X.Oli reset>
                <X.P>模拟标志寄存器入栈</X.P>
