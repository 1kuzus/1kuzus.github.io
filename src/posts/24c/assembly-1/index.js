...
export default function Post() {
    return (
        <>
            ...
            <X.P>
                8086
                CPU所有寄存器都是`16`位的，可以存放`2`个字节。有时还会见到`AH`、`AL`的表示方法，它们分别表示`AX`的高、低`8`位。
            </X.P>
            <X.H1>8086 CPU物理地址表示</X.H1>
            <X.P>
                8086
                CPU有`20`位地址总线，最大寻址空间是`1MB`，但由于字长是`16`位，所以需要两个`16`位地址（*段地址*、*偏移地址*）相加合成一个`20`位地址。
            </X.P>
            ...
            <X.P>
                8086
                CPU执行程序时，`CS`寄存器存放的是代码段的段地址，`IP`寄存器存放的是代码段中的偏移地址。\nCPU将内存中`CS:IP`指向的内容当作指令执行。
            </X.P>
            <X.P>
                既然执行何处的指令取决于`CS:IP`，那么就需要有一种方法可以改变`CS`和`IP`的值，这就是`jmp`指令。8086
                CPU不支持用立即数修改`CS`和`IP`的值（例如`mov cs,2000`），但是可以通过转移指令`jmp`来实现。
            </X.P>
            <X.Uli>
                <X.P>
                    同时修改`CS`、`IP`的内容：`jmp
                    段地址:偏移地址`，用指令给出的段地址修改`CS`，偏移地址修改`IP`，例如：
                </X.P>
                <X.CodeBlock
                    language="asm8086"
                    code={`
            ...
            <X.P>
                8086 CPU也可以一次性传输一个字，假如`DS`寄存器的值为`1000h`，则`mov
                ax,[0]`就表示将`1000:0`中的字送入`AX`中；`mov [0],cx`就表示将`CX`中的字送入`1000:0`中。
            </X.P>
            <X.H1>栈操作（push指令、pop指令）</X.H1>
            <X.P>
                基于8086 CPU编程，可以将一段内存当作栈来使用。8086
                CPU有两个与栈相关的寄存器，`SS`存放栈顶的段地址，`SP`存放栈顶的偏移地址。在任意时刻，`SS:SP`指向栈顶元素。
            </X.P>
            ...
            <X.HighlightBlock>
                <X.P>
                    8086
                    CPU只知道栈顶的位置，不知道栈的大小有多大，并不能保证栈操作不会越界。编程时需要程序员主动避免栈溢出风险。
                </X.P>
            </X.HighlightBlock>
