...
export default function Post() {
    return (
        <>
            ...
            <X.Oli reset>
                每个人的输入习惯都有很大差别，因此很难设定一个绝对的时间间隙作为标准；因此研究使用相对指标`z score`，如果向量{`$\\bm{t}$`}代表所有按键的时间间隔，那么`z score`为
                {`$\\frac{\\bm{t}-\\mu_{\\bm{t}}}{\\sigma_{\\bm{t}}}$`}
                ，即“与均值相差几个标准差”。然后进一步定义了一个`z score`的阈值，使得大小写切换`(case switches)`真阳性率$TRPCS$和非大小写切换`(non case switches)`真阳性率$TPRNCS$的乘积最大。
            </X.Oli>
            ...
            <X.P>上图中的红色箭头标识一个键盘切换`(keyboard switches)`（有些复杂的密码可能需要在字母键盘和符号键盘等等之间反复切换），每次检测到键盘切换后，会为相同的当前密码创建三个不同状态的副本以覆盖所有的可能性。</X.P>
