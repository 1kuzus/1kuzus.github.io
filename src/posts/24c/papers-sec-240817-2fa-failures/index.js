...
export default function Post() {
    return (
        <>
            <X.P>相关链接：</X.P>
            <X.Uli>
                @Security and Privacy Failures in Popular 2FA
                Apps[https://www.usenix.org/system/files/sec23summer_198-gilsenan-prepub.pdf]@
            </X.Uli>
            <X.HighlightBlock background="blue">
                ...
                <X.P>
                    SMS 2FA是平时最熟悉的短信验证码，它的操作比TOTP 2FA更为简单，但是安全性不如TOTP
                    2FA。首先，存在针对短信的攻击手段；其次，SMS
                    2FA的可靠性基于移动运营商（认证设备被停机、断网、信号不好都无法正常工作），而TOTP只需要认证设备本地存储的密钥（并且不需要认证设备联网）。
                </X.P>
                <X.H3>PII</X.H3>
                <X.P>
                    个人可识别信息`(Personally Identifiable Information,
                    PII)`是可以用来识别某人的任何数据，所有直接或间接与个人相关的信息都被视为PII，例如一个人的姓名、电子邮件地址、电话号码、银行账号和政府颁发的身份证号码等。
                </X.P>
            ...
            <X.P>
                TOTP是一种常见的2FA形式，通常被认为比SMS 2FA更安全；然而TOTP
                2FA也为用户带来了一下易用性负担，用户常常丢失设备、更换设备或卸载TOTP应用，如果无法生成OTP就无法登录账户。为了应对这种易用性问题，许多TOTP应用程序提供自定义备份机制，以帮助用户从设备丢失中恢复。论文主要研究这些备份机制的安全问题和隐私问题：
            </X.P>
            ...
            <X.P>
                在启用TOTP
                2FA之前，用户通常被要求使用TOTP应用程序扫描一个QR码，这样做的目的是与TOTP应用程序共享三个关键信息：
            </X.P>
            ...
            <X.Oli>
                Exploring the
                App：探索、记录各种功能和设置，例如是否需要个人信息才能使用、支持何种备份机制等等，然后启用备份机制，最终执行恢复过程。
            </X.Oli>
            ...
            <X.P>
                Android
                6.0及更高版本支持自动将应用程序数据上传到用户的谷歌云端硬盘的备份系统。默认情况应用程序会加入AAB，但官方文档建议如果应用程序需要处理Android不应该自动备份的敏感信息，就应该设置`android:allowBackup="false"`来退出AAB。数据集中`12`个应用程序明确声明了退出AAB。
            </X.P>
            <X.H2>备份机制的隐私影响</X.H2>
            <X.P>
            ...
            <X.P>
                论文这一节的内容的中心思想是：TOTP 2FA本意是想取代安全性较差的密码登录、SMS
                2FA等机制，但由于账户恢复这一难题，对于云同步的TOTP备份机制，用户仍然使用密码、SMS等方式进行恢复时的身份验证，从而把安全性又降回到这些传统机制上。
            </X.P>
