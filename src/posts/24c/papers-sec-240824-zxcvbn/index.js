    ...
    return (
        <>
            ...
            <X.Uli>
                ...
                Estimation[https://www.usenix.org/system/files/conference/usenixsecurity16/sec16_paper_wheeler.pdf]@
            </X.Uli>
            ...
            <X.H2>LUDS</X.H2>
            <X.P>
                一种常见的密码强度估计方案：利用小写字母`(*L*owercase)`、大写字母`(*U*ppercase)`、数字`(*D*igit)`和特殊字符`(*S*ymbol)`的出现次数来评估密码强度，简写为LUDS，这种策略可以追溯到NIST（美国国家标准与技术研究院）的标准。作者认为这种估计方案从根本上就是无效的——`Passw0rd`因为有数字`0`，反而会比`QJqUbPpA`这样明显随机的密码拥有更高的分数；密码中常见的模式，如日期、键盘规律（比如`qwerty`）无法在分数中有体现。
            </X.P>
            <X.P>NIST的标准（通常叫NIST熵）伪代码见下图：</X.P>
            <X.Image src="1.jpg" width="500px" filterDarkTheme />
            <X.P>
                前两条都是LUDS规则：根据密码长度和特殊字符加分；尽管第三条（是可选的）考虑了字典攻击，前面的例子`Passw0rd`（假设它没通过字典测试）依然会得到和`QJqUbPpA`（假设它通过了）一样的分数，这是令人困惑的。
            </X.P>
            <X.H2>密码猜测相关</X.H2>
            <X.P>
                本文使用的“黄金准则”（原文是`gold standard`，含义类似于Ground
                Truth，在后面的实验用于验证强度估计的准确性）是利用Ur等人提出的@Password Guessability
                Service[https://pgs.ece.cmu.edu/]@（论文在@这里[https://www.usenix.org/system/files/conference/usenixsecurity15/sec15-paper-ur.pdf]@，后面简称PGS）得到的面对四种现代密码猜测攻击中，需要进行的最少限度的尝试。
            </X.P>
            <X.P>
            ...
            <X.P>
                对于一个旨在替代LUDS的方案，这一节讨论了各方面的评价标准。作者参考了两种LUDS方法：NIST熵和`3class8`（一种易于采用的要求，即密码包含至少`3`种类型的`8`个或更多字符）。
            </X.P>
            ...
            <X.Uli>
                ...
                <X.P>
                    在线攻击同样是一个重要威胁，能够在约几百次内猜测出的密码对于在线攻击显然是脆弱的，但NIST熵会严重高估这种密码的安全性。作者将$10^6$作为在线攻击的阈值（认为需要猜测$10^6$以上次的密码对于在线攻击相对安全），并希望新的方案能够在阈值之前保持较高的准确性。
                </X.P>
            </X.Uli>
            ...
            <X.P>
                对于一个给定的密码字符串，可以匹配到多种模式，并且这些模式会有重叠，例如密码`lenovo1111`可能提取到`lenovo(token)`、`eno(reversed
                'one')`、`1111(repeat)`、`1111(date of 2011/1/1)`等模式。
            </X.P>
            ...
            <X.P>
                这是一个`对数 -
                对数`坐标，$x$轴是前面提到的黄金准则PGS，$y$轴是估计；在$10^6$处的竖线是在线攻击的阈值；越接近$y=x$这条实线说明密码强度估计与PGS得到的结果越接近，虚线是一个大致不错的范围。
            </X.P>
            <X.P>
            ...
            <X.HighlightBlock>
                <X.P>
                    看到各图中都有很多水平的“条带”，一般来说LUDS方法对具体字符的不敏感的，比如`hello`、`abcde`、`vdskj`很可能具有相同的分数，此时大量的密码都对应了同一个估计分数，但是PGS指标可能相差较多，就形成了一条水平线；
                </X.P>
                <X.P>
                    本文提出的zxcvbn，在中高数量级也出现了条带状，这对应着`bruteforce`模式（没有其他模式匹配得上，只能暴力）。
                </X.P>
            </X.HighlightBlock>
            <X.H2>其他</X.H2>
            <X.P>
                论文还做了关于数据集大小选择（`100k`/`10k`/`1k` token
                sets）或只考虑某一部分模式对结果的影响，以及运行性能分析，具体见论文原文。
            </X.P>
