...
export default function Post() {
    return (
        <>
            <X.P>相关链接：</X.P>
            <X.Uli>
                @Maginot Line: Assessing a New Cross-app Threat to PII-as-Factor Authentication in Chinese Mobile
                Apps[https://www.ndss-symposium.org/wp-content/uploads/2024-241-paper.pdf]@
            </X.Uli>
            <X.H1>1.Introduction</X.H1>
            <X.P>
                个人可识别信息`(Personally Identifiable Information,
                PII)`（如身份证号）在中国的移动应用程序中被广泛用作身份认证因素，在本文中这种身份认证方法被称为PaFA`(PII-as-Factor
                Authentication)`。PaFA旨在提供更强的身份认证限制。不过对于不同类型的软件，身份认证的限制也会不同。例如相比于银行App，电子书软件的身份认证通常限制更小。
            </X.P>
            <X.P>
                论文发现了一个新的威胁：在中国，PII被移动应用程序供应商广泛收集和存储；如果攻击者能够获得用户的SMS
                OTP并登录一些认证强度低的App，就可以通过在这些App上收集用户的PII，最终绕过目标App的PaFA认证机制。
            </X.P>
            <X.HighlightBlock>
                <X.P>
                    跨程序认证绕过（原文是`Bypass Authentication by Cross App Exploitation`，简写为`Bacae
                    Attack`）的一个实例是：支付宝本来需要四个身份认证因素：SMS
                    OTP、银行卡号、全名和身份证号；但如果是通过淘宝授权登录，攻击者只需要两个认证因素登录淘宝：SMS
                    OTP和身份证号。
                </X.P>
            </X.HighlightBlock>
            <X.P>论文的主要贡献是：</X.P>
            <X.Uli>
                发现了用户在同时使用多个小程序的情况下，可能会对其他应用程序产生重要安全影响，使得PaFA认证机制的强度在大多数情况下仅与SMS
                OTP相同。
            </X.Uli>
            <X.Uli>
            ...
            <X.H2>威胁模型</X.H2>
            <X.P>
                用户A使用了很多热门应用程序，并提供了真实的PII，这些应用程序也保存了A的使用历史记录（如历史订单）。目标应用程序供应商的安全预期是，仅仅拥有不充分的身份验证因素（例如只有SMS
                OTP）是不足以完成应用程序的密码恢复过程或身份验证过程的。
            </X.P>
            <X.P>攻击者M仅能截获A的SMS OTP。M旨在收集A的PII，然后绕过目标应用程序的身份验证机制。</X.P>
            <X.H1>3.Design and Implementation of MAGGIE</X.H1>
            <X.P>
                MAGGIE包含三个核心模块：`XHelper`用于从UI中获取有用的PII，`Model Builder`生成状态机模型，`Model
                Checker`进行分析并输出攻击路径。
            </X.P>
            ...
            <X.P>
                使用一个状态机对攻击过程进行建模：$M=(S,O,T,s_0,s_s)$，$S$是状态集，$s_0$是初始状态（攻击者此时仅有不充分的认证因素），$s_s$是成功状态（攻击已经达成），$O$是能在应用程序上进行的操作的有限集，$T$是状态转移函数（例如$T(s_i,
                App_x.login)=s_j$）。
            </X.P>
            ...
            <X.P>
                实验过程中研究人员首先用个人真实信息手动（因为通常需要人机测试）注册和配置应用程序；然后使用`XHelper`探索应用程序，如果过程中出现人机测试同样手动协助它完成；然后基于`Model
                Builder`生成的模型，使用`Model
                Checker`生成对于每个`App.Op`的`1000`个优化的攻击路径；最后由研究人员手动验证。
            </X.P>
            <X.HighlightBlock>
            ...
            <X.Uli>
                <X.P>
                    应用程序中无处不在的PII：只能访问到SMS
                    OTP的攻击者可以从身份认证强度低的其他应用程序中收集PII，然后破坏更强的PaFA系统。
                </X.P>
                <X.P>一个真实的案例：</X.P>
            ...
            <X.Uli>
                ...
                <X.Uli>
                    <X.P>
                        账户共享：SSO（如淘宝和支付宝的例子），或者大公司的产品共享（如Google账户可以登录Gmail和Google
                        Calendar），如果服务的安全设计不一致就会破坏预期的强身份认证。
                    </X.P>
                    <X.HighlightBlock>
                        <X.P>
