...
export default function Blog() {
    return (
        <X.BlogWrapper>
            ...
            <X.HighlightBlock bgcolor="gray">
                <X.H3>重新理解</X.H3>
                <X.P>重述上面的故事，以免迷失在符号的海洋里。</X.P>
                <X.P>
                    假设我们拿到了正态分布下的样本集{`$\\bm{\\chi}=\\{x_1,x_2,x_3\\}$`}
                    ，其中$x_1=1.3$，$x_2=4.1$，$x_3=3.7$，分布的方差已知为`3`，现在只需要估计均值$\mu$。\n
                    样本集中的每个样本出现的概率$P(x_i|\mu)$是可以由正态分布写出的，把所有样本的出现概率相乘，就得到了观察到当前样本集的概率为
                    {`$P(\\bm{\\chi}|\\mu)$`}，这也就是上述`步骤2`提到的`样本集分布`。
                </X.P>
                <X.P>
                    注意在这个问题中，只有一个参数需要估计，而样本观测值是已知的，那么样本集分布最后是一个只含$\mu$的表达式。因此我们把这个表达式记作$f(\mu)$，也就是有
                    {`$f(\\mu) = P(\\bm{\\chi}|\\mu)$`}
                    。\n例如样本集有三个样本$x_1=1.3$，$x_2=4.1$，$x_3=3.7$，前面提到的已知的正态分布方差为`3`，那么有：
                </X.P>
                <X.Formula
                    text="f(\mu) =
                ...
                <X.P>
                    首先来看分母$\int_0^5 f(\mu) P(\mu) d\mu$，其含义是综合考虑所有可能的$\mu$取值，---
                    求出一个“平均的”样本集出现概率。这个积分式可以求出具体数值，而不是含$\mu$的式子。从数学的角度可以理解为，使等号左侧后验概率密度积分为`1`的归一化常数。
                </X.P>
                <X.P>
                    现在来看其他的三个量，{`$P(\\mu|\\bm{\\chi})$`}、{`$f(\\mu)=P(\\bm{\\chi}|\\mu)$`}、$P(\mu)$，---
                    这是贝叶斯学派的经典思想：用样本修正先验概率，得到后验概率。这三个式子都可以写为仅含$\mu$的函数。当然，因为假设的先验分布简单，$P(\mu)$为常数。\n
                    这三个式子虽然都是$\mu$的函数，但描述的含义有所区别。{`$P(\\mu|\\bm{\\chi})$`}
                    、$P(\mu)$描述的是$\mu$的分布概率，而$f(\mu)$描述的是样本集的出现概率。
                </X.P>
                <X.P>
                    与最大似然估计一样，贝叶斯估计也会给出参数具体的估计值。但到此步，我们只给出了$\mu$的后验分布。要给出估计值，首先要考虑我们的目标是什么：\n
                    最大似然估计的目标是，带入估计值可以使得样本集出现概率$f(\mu)$最大；\n
                    而贝叶斯估计的目标是，带入估计值可以使得期望风险（定义见上文）最小。\n
                    进一步地，在损失函数为平方误差时，有结论可以给出估计参的数具体表达式，也就是`步骤4`。
                </X.P>
            </X.HighlightBlock>
            <X.H2>贝叶斯学习</X.H2>
