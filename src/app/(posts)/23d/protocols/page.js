...
export default function Post() {
    return (
        <>
            ...
            <X.P noMarginBottom>路由表中包含*目的网络*、*到目的网络的距离*、*下一跳路由器*。对于每个路由器，路由表更新规则为：</X.P>
            ...
            <X.Table>
                ...
                <tr>
                    ...
                    <td>否</td>
                    <td>
                        <X.P>`0.0.0.0`严格来说不是一个真正意义上的ip地址，\n它在本网范围内表示主机，也被用于一些协议的默认参数。</X.P>
                    </td>
                </tr>
                <tr>
                    <td>
            ...
            <X.HighlightBlock>
                ...
                <X.P noMarginBottom>流量控制中接收方告知发送方的窗口大小称为*接收窗口*`(Receiver Window, rwnd)`，其反映了接收方的容量；\n拥塞控制中发送方根据网络拥塞程度调整的发送窗口大小称为*拥塞窗口*`(Congestion Window, cwnd)`，其反映了当前网络容量。</X.P>
