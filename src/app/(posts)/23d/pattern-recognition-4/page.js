...
export default function Post() {
    return (
        <>
            ...
            <X.H1>背景知识</X.H1>
            <X.H2>Sigmoid函数</X.H2>
            <X.P>`Sigmoid`函数通常表示为{`$\\sigma(x)=\\frac{1}{1+e^{-x}}$`}，值域为$[0,1]$，有以下两个常用的性质：</X.P>
            <X.Formula text="\sigma(x)+\sigma(-x)=1" />
            <X.Formula text="\sigma'(x)=\sigma(x)\cdot\sigma(-x)=\sigma(x)\cdot[1-\sigma(x)]" />
            <X.H2>常数问题</X.H2>
            <X.P>
                你可能看到过{`$y=\\bm{W}^T\\bm{X}+b$`}和{`$y=\\bm{W}^T\\bm{X}$`}两种计算分类结果的方式。以二维空间上的分类为例，前者的分类线可以是任意直线，而后者只能是过原点的直线。事实上，可以通过将问题空间升一个维度来统一两种形式。
            </X.P>
            <X.P>
                令{`$\\bm{W} \\leftarrow [\\bm{W} \\; b]$`}，{`$\\bm{X} \\leftarrow [\\bm{X} \\; 1]$`}，两种形式即可统一。
            </X.P>
            <X.P>上述操作相当于，仍然以二维空间上的分类为例，若希望分类线不受过原点的限制，考虑把问题放入三维空间：\n$xy$坐标不变，令$z=1$，此时可以在三维空间中用一过原点的平面作为分类面，这个分类面与平面$z=1$的交线可以是任意（平面$z=1$上的）直线。</X.P>
            <X.H1>线性回归</X.H1>
            <X.H2>动机</X.H2>
            <X.P>用下式：</X.P>
            ...
            <X.H2>动机</X.H2>
            <X.P>线性二分类问题都可以看作把样本投影到某个方向上，再在这个一维空间中确定一个阈值，将两类样本分开。现在希望找到的方向满足：两类间的距离尽可能大，但每一类内部的样本尽可能聚集。</X.P>
            <X.H2>准则函数</X.H2>
            <X.P>
            ...
            <X.H2>动机</X.H2>
            <X.P>
                使用`Sigmoid`函数将{`$\\bm{W}^T\\bm{X}$`}归约到区间$[0,1]$，可以把这个输出结果看作概率，那么输出将从单纯的以{`$\\bm{W}^T\\bm{X}$`}的符号进行二分类变成连续的概率。
            </X.P>
            <X.H2>损失函数</X.H2>
            ...
            <X.P>假设`Sigmoid`函数的输出是$p$，其含义应为“样本是正类”的概率。因此一个正样本对似然函数的贡献是$p$，而负样本要代入$1-p$。换句话说，上述对数似然函数中的{`$P(y_i|\\bm{X}_i)$`}可以有如下数学表示：</X.P>
            <X.Formula
                text="P(y_i|\bm{X}_i)=
                \begin{cases}
