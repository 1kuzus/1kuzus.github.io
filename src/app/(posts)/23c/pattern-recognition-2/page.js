...
const path = '/23c/pattern-recognition-2/';
    ...
    return (
        <>
            ...
            <X.HighlightBlock bgcolor="gray">
                ...
                <X.Uli>
                    <X.P>样本集中的每个样本出现的概率$P(x_i|\mu)$是可以由正态分布写出的，把所有样本的出现概率相乘，就得到了当前样本集出现的概率为{`$P(\\bm{\\chi}|\\mu)$`}，这也就是上述`步骤2`提到的`样本集分布`。</X.P>
                    ...
                    />
                    <X.P>
                        如果采用极大似然估计，那么做到这里对$f(\mu)$或$\ln f(\mu)$求导，使得导数为`0`的点就是估计值{`$\\hat{\\mu}$`}；取{`$\\mu=\\hat{\\mu}$`}可以使得观测到样本集的概率$f(\mu)$最大。
                    </X.P>
                </X.Uli>
                <X.Uli>
                ...
                <X.Uli>
                    ...
                    <X.P>
                        再看其他的三个量，{`$P(\\mu|\\bm{\\chi})$`}、{`$f(\\mu)=P(\\bm{\\chi}|\\mu)$`}、$P(\mu)$，这是贝叶斯学派的经典思想：用样本修正先验概率，得到后验概率。这三个式子都可以写为仅含$\mu$的函数。当然，因为假设的先验分布简单（均匀分布），$P(\mu)$为常数。\n这三个式子虽然都是$\mu$的函数，但描述的含义有所区别。{`$P(\\mu|\\bm{\\chi})$`}、$P(\mu)$描述的是$\mu$的分布概率，而$f(\mu)$描述的是样本集的出现概率。
                    </X.P>
                </X.Uli>
                <X.Uli>
                    <X.P>与最大似然估计一样，贝叶斯估计也会给出参数具体的估计值。但到此步，我们只给出了$\mu$的后验分布。要给出估计值，首先要明确我们的目标是什么：\n最大似然估计的目标是，带入估计值可以使得样本集出现概率$f(\mu)$最大；\n而贝叶斯估计的目标是，带入估计值可以使得期望风险（定义见上文）最小。\n进一步地，在损失函数为平方误差时，有结论可以给出估计值{`$\\hat{\\mu}$`}的具体数值，也就是`步骤4`。</X.P>
                </X.Uli>
            </X.HighlightBlock>
            <X.H2>贝叶斯学习</X.H2>
            ...
            <X.P>
                现在继续观测到第二个样本{`$\\bm{X}_2$`}，此时根据贝叶斯学派*用数据修正先验*的思想，先验分布是上一轮得到的{`$P(\\bm{\\theta}|\\bm{X}_1)$`}，修正后的后验分布为
            </X.P>
