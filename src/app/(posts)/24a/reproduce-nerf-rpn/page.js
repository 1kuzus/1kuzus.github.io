...
const path = '/24a/reproduce-nerf-rpn/';
...
export default function Post() {
    return (
        <>
            ...
            <X.P>作者修改了Instant-NGP的仓库，添加了一些包括可视化bbox在内的功能。说明文档在@这里[https://github.com/zymk9/instant-ngp/tree/master/scripts]@。</X.P>
            <X.CodeBlock language="bash" code="git clone --recursive https://github.com/zymk9/instant-ngp.git InstantNGPforked" />
            <X.P>这是Instant-NGP一个早期版本，执行下面的命令运行`build`过程。更多细节可以参考@这里[https://github.com/zymk9/instant-ngp]@。</X.P>
            <X.CodeBlock
                language="bash"
                code={`
                cd InstantNGPforked
            ...
            <X.CodeBlock
                ...
                `}
            />
            <X.P>执行上述命令后，勾选UI`Debug visualization`下的`Visualize object bounding boxes`就可以看到如下结果：</X.P>
            <X.Image src="fig1.jpg" width="100%" />
            <X.H2>下载预训练的权重</X.H2>
            <X.P>本博客给出的示例需要在上面的OneDrive链接中下载`nerf_rpn_model_release\front3d_anchor_resnet50.pt`。</X.P>
            ...
            <X.CodeBlock
                ...
                `}
            />
            <X.P>运行后会在`NeRFrpn\\nerf_rpn\\results\\proposals_to_ngp`中得到`17`个测试场景的`3dfront_xxxx_xx.json`文件。</X.P>
            <X.P>接下来，把文件`3dfront_0004_00.json`复制到路径`E:\\front3d_nerf_data\\3dfront_0004_00\\train`下，然后启动Instant-NGP的UI并指定这个新的`json`文件：</X.P>
            <X.CodeBlock
                language="bash"
                code={`
                cd InstantNGPforked
            ...
            <X.H1>文件结构参考</X.H1>
            <X.P>这里展示了前面我们生成、或需要用到的各个文件的路径示意。注意这并不是仓库中的全部内容，而是只展示了前面提及的文件。</X.P>
            <X.CodeBlock
                language="bash"
                code={`
                <work_dir>
