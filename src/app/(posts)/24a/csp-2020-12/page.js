...
export default function Post() {
    return (
        <>
            ...
            />
            <X.H1>T4 食材运输</X.H1>
            <X.P>这题大概分成两步做：\n第一步，树上`DFS`求对于特定的食材，选取特定的检查点，*等待时间最长的酒店等待的最短时间*；\n第二步，二分求答案，`check(x)`函数为能否在`m`个检查点之内使得对于所有食材，*等待时间最长的酒店等待的最短时间*的最大值小于等于`x`。</X.P>
            <X.H2>第一步：树上DFS</X.H2>
            <X.HighlightBlock>
                <X.P>每次`DFS`都是针对某种特定的食材`j`的，体现为代码中的`int dfsReturn(int root,int j)`、`int dfsSingle(int root,int j)`，但是下面为了便于描述，忽略这个食材种类`j`了。</X.P>
            </X.HighlightBlock>
            <X.P>树上`DFS`做两次：</X.P>
            <X.P>第一次记录*往返时间*，也就是从当前酒店出发遍历子树所有的需要某种食材的酒店*再回到当前酒店*的时间；\n下面公式中，记当前酒店*含有需要这种食材的酒店*的子树为$i$，$w$是边权：</X.P>
            <X.Formula text="t_{return}(root)=\sum_i (t_{return}(i) + 2w(root,i))" />
            <X.P>对于判断子树究竟含不含有需要这种食材的酒店，只需要看子树的{`$t_{return}$`}非零，或者子树的根节点直接需要这种食材。</X.P>
            <X.P>第二次记录*单程时间*，也就是从当前酒店出发遍历子树所有的需要某种食材的酒店，等待时间最长的酒店等待的最短时间。\n考虑单程时间相比往返时间减少的部分，假如从当前节点$root$选择的最后走一个子树分支$i$，就不需要再从此路返回了。因此首先节省了一倍$w(root,i)$的时间；其次，对于遍历这颗子树花费的时间，也从往返时间变成了单程时间，因此还可以节省{`$t_{return}(i)-t_{single}(i)$`}的时间。因此有：</X.P>
            ...
            <X.P>最小值最大或最大值最小的问题可以用二分求解，此题二分的`check`函数实质上是一个重复覆盖问题，可以用状压`DP`求解，具体见代码注释。</X.P>
            <X.H2>代码</X.H2>
            <X.CodeBlock
                language="cpp"
                code={`
            ...
            <X.H1>T5 星际旅行</X.H1>
            <X.H2>运算符重载简化编码</X.H2>
            <X.P>线段树题，考虑到对三个坐标分开维护较为麻烦，考虑重载自定义`POS`类型的运算符，隐去三个坐标的内部逻辑，当作整体考虑：</X.P>
            <X.CodeBlock
                language="cpp"
                code={`
                struct POS{
                ...
                `}
            />
            <X.P>对于要维护的转向操作，实际上逻辑类似对`(x,y,z)`进行循环左移，这里就重载左移运算符来表示。根据题意，每次操作只会左移至多`1`位。</X.P>
            <X.H2>线段树懒标记优先级</X.H2>
            <X.P>线段树题中有一种类就是会有多个懒标记，需要我们指定优先级，例如经典的“先乘后加”@洛谷P3373【模板】线段树2[https://www.luogu.com.cn/problem/P3373]@。此题涉及了三种运算：乘、加、左移，需要保证先乘后加，左移的优先级任意。这里以*左移优先级最后*为例分析懒标记的合并。</X.P>
            <X.HighlightBlock>
                <X.H3>个人理解</X.H3>
            ...
            <X.P>如果不想看推导过程，只需要从`p=(x,y,z)`出发，验证只经过一次上述操作（先乘`ag`，再加`bg+(h&lt;&lt;(3-c))`，再左移`(c+i)%3`），即可得到`p=(g(az+b3)+h2,g(ax+b1)+h3,g(ay+b2)+h1)`。</X.P>
            <X.P>对应在代码中就是：</X.P>
            <X.CodeBlock
                language="cpp"
                code={`
