    ...
    return (
        <>
            ...
            <X.Oli>
                ...
                <X.P noMarginBottom>答：激活函数为神经网络引入了非线性，能提高网络对模型的表达能力，解决线性模型所不能解决的问题。</X.P>
            ...
            <X.Br />
            <X.Oli>
                <X.P noMarginBottom>考虑某个具体问题时，你可能只有少量数据来解决这个问题。不过幸运的是你有一个类似问题已经预先训练好的神经网络。可以用下面哪种方法来利用这个预先训练好的网络？</X.P>
            ...
            <X.HighlightBlock bgcolor="red">
                ...
                <X.Table>
                    ...
                    <tr>
                        <th>相似度低</th>
                        <td>
                            <X.P>主干部分提取的特征是通用特征，可以冻结预训练模型的初始层（比如`k`层），并再次训练剩余的提取高层特征`n-k`层。</X.P>
                        </td>
                        <td>
            ...
            <X.Br />
            <X.Oli>
                <X.P noMarginBottom>对于一个分类任务，如果开始时神经网络的权重不是随机赋值的，而是都设成$0$，下面哪个叙述是正确的？</X.P>
            ...
            <X.Br />
            <X.Oli>
                <X.P noMarginBottom>当训练一个神经网络来作图像识别任务时，通常会绘制一张训练集误差和交叉训练集误差图来进行调试。在图中，最好在哪个时间停止训练？</X.P>
            ...
            <X.Br />
            <X.Oli>
                <X.P noMarginBottom>如果我们希望预测$n$个类的概率使得所有概率和等于$1$，那么下列哪个函数可以用作输出层中的激活函数？</X.P>
            ...
            <X.Oli>
                ...
                <X.Uli>VGG：提出了可以通过重复使用简单的基础块来构建深度模型的思路。使用三个`3`\*`3`卷积核代替`7`\*`7`卷积核，使用两个`3`\*`3`卷积核代替`5`\*`5`卷积核，在保证具有相同感知野的条件下，提升了网络的深度，减小了网络参数，在一定程度上提升了神经网络的效果。</X.Uli>
            ...
            <X.Oli>
                ...
                <X.Uli>
                    <X.P noMarginBottom>梯度消失，JS散度的问题：只要两分布没有重叠部分或重叠部分可忽略，JS散度恒为$\log2$，目标函数为$0$。当判别器最优时，生成器无法获得任何梯度信息，梯度消失，无法训练。</X.P>
            ...
            <X.Br />
            <X.Oli>
                <X.P noMarginBottom>设{`$A=\\{上,下,左,右\\}$`}为动作空间，$s_t$为当前状态，$Q^\ast$为最优动作价值函数，其输出：\n$Q^\ast(s_t,上)=930$\n$Q^\ast(s_t,下)=-60$\n$Q^\ast(s_t,左)=120$\n$Q^\ast(s_t,右)=321$\n请问，哪个动作会成为$a_t$?</X.P>
            ...
            <X.Oli>
                ...
                <X.P noMarginBottom>答：源域是模型在训练阶段接触到的领域，通常具有相对丰富的标记数据。目标域是模型在测试阶段面对的领域，通常没有或有很少的标记数据。将源域和目标域的差异性视为两个域内样本数据的概率分布的差异性进行研究，根据采用显式或隐式的距离度量分为两类：</X.P>
            ...
            <X.Oli>
                ...
                <X.P withMarginTop>孪生网络是一种特殊的神经网络结构，有两个相同的子网络，它们共享相同的权重。这两个子网络分别处理输入样本，然后通过一个比较层将两个子网络的输出进行比较。孪生网络的训练目标是使得相似的样本在比较层中的距离较小，而不相似的样本的距离较大。对比损失是孪生网络中常用的损失函数。</X.P>
