...
const path = '/24a/games101-03-shading/';
...
export default function Post() {
    return (
        <>
            ...
            <X.H1>纹理映射</X.H1>
            <X.P>纹理映射希望能够定义一个物体上面任意一点的不同属性（$k_d$等）。认为3D物体的表面是一张2D的图，纹理映射需要建立起3D物体表面上一点与纹理表面上的一点的一一对应关系。</X.P>
            ...
            <X.H2>凹凸贴图</X.H2>
            <X.FlexRow gap="32px">
                <X.Image src="fig10.jpg" style={{minWidth: '200px'}} />
                <X.P>纹理不止可以表示颜色，也可以表示物体的模型。例如，一个球的表达是非常容易的，通常几百个三角形就能很好的描述，但是左图所示的表面有复杂纹理的球体如果用三角形表示，则很麻烦。可以用像描述表面纹理一样的方法去描述每一点的相对高度。</X.P>
            </X.FlexRow>
            <X.FlexRow gap="32px">
                <X.Image src="fig11.jpg" width="200px" style={{minWidth: '200px'}} />
            ...
            <X.P>由于凹凸贴图和法线贴图`(左图)`并不会真正的改变顶点，在一些细节上会与位移贴图`(右图)`有出入，例如物体的边缘和凸起部分的阴影。</X.P>
            <X.P>位移贴图要求原图三角形的定义足够细致，也就是原物体顶点的采样率要高于凹凸纹理的采样率，否则就没办法通过移动顶点来实现纹理（就像很低分辨率的屏幕上不能很好的显示出高分辨率的图片）。</X.P>
            <X.H2>3D纹理</X.H2>
            <X.Image src="fig14.jpg" width="600px" />
            <X.P>之前讨论的纹理都是2D的，但纹理也可以定义在3D空间中，例如上图的纹理是定义在3D空间中的噪声，空间中每一个点都有值（把花瓶切开，其内部也是有纹理的）。</X.P>
