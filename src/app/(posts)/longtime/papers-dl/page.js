    ...
    return (
        <>
            ...
            <X.HighlightBlock bgcolor="blue">
                ...
                <X.P>R-CNN的速度很慢，因为可能从一张图像中选出上千个提议区域，这需要上千次的卷积神经网络的前向传播来执行目标检测。这种庞大的计算量使得R-CNN在现实世界中难以被广泛应用。</X.P>
                <X.Image src="rcnn.jpg" width="600px" />
                <X.H3>Fast R-CNN</X.H3>
                <X.P>R-CNN的主要性能瓶颈在于，对每个提议区域，卷积神经网络的前向传播是独立的，而没有共享计算。由于这些区域通常有重叠，独立的特征抽取会导致重复的计算。Fast R-CNN的主要改进之一，是仅在整张图象上执行卷积神经网络的前向传播，并且引入*兴趣区域池化*`(ROI Pooling)`，将卷积神经网络的输出和提议区域作为输入，输出连结后的各个提议区域抽取的特征。</X.P>
                <X.P>兴趣区域池化层可以给出固定大小的输出：把给定的锚框均匀分割成$n \times m$块，输出每块里的最大值，这样无论锚框多大，总是输出$nm$个值。</X.P>
                <X.P>Fast R-CNN先对图片用CNN抽取特征，然后将`selective search`给出的原图上的提议区域映射到CNN特征图上，再经过`ROI Pooling`就可以得到维度对齐的特征。</X.P>
                <X.Image src="fastrcnn.jpg" width="400px" />
                <X.H3>Faster R-CNN</X.H3>
                <X.P>与Fast R-CNN相比，Faster R-CNN将生成提议区域的方法从`selective search`改为了*区域提议网络*`(Region Proposal Network, RPN)`，模型的其余部分保持不变。区域提议网络作为Faster R-CNN模型的一部分，是和整个模型一起训练得到的。换句话说，Faster R-CNN的目标函数不仅包括目标检测中的类别和边界框预测，还包括区域提议网络中锚框的二元类别和边界框预测。作为端到端训练的结果，区域提议网络能够学习到如何生成高质量的提议区域，从而在减少了从数据中学习的提议区域的数量的情况下，仍保持目标检测的精度。</X.P>
            ...
            <PaperSummary
                ...
                method={
                    <>
                        <X.P>基于Faster R-CNN，添加一个对每个ROI预测分割mask的分支。这个分支可以与分类和边界框预测分支并行。</X.P>
            ...
            <X.HighlightBlock bgcolor="blue">
                ...
                <X.H3>FCN中的转置卷积</X.H3>
                <X.P>例如对于ImageNet的图片输入，大小通常为`224`\*`224`\*`3`（RGB通道）；经过卷积后缩小宽高缩小`32`倍，通道增加到`512`，变成`7`\*`7`\*`512`的特征图。此时FCN会先通过一个`1x1conv`进行通道降维，然后通过转置卷积将特征图的高度和宽度增加`32`倍，*输出通道数等于类别数*，相当于储存了对每一类的预测结果。</X.P>
            </X.HighlightBlock>
        </>
    );
}
