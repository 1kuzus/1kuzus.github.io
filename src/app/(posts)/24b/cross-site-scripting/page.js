...
export default function Post() {
    return (
        <>
            ...
            <X.P>*跨站脚本攻击*`(Cross-site Scripting, XSS)`是一种常见的网络安全漏洞，攻击者通过在网页中插入恶意脚本，使得用户在浏览网页时执行这些脚本，从而达到攻击的目的。</X.P>
            <X.P>XSS攻击分为三种：反射型、存储型、DOM型。反射型和存储型是服务器端的漏洞，DOM型是客户端的漏洞。</X.P>
            <X.H2>Reflected XSS</X.H2>
            <X.P>反射型`(Reflected XSS)`/非持久型`(Non-Persistent XSS)`：服务器直接从HTTP请求中读取数据，并且返回到HTTP响应中。</X.P>
            ...
            <X.P>存储型`(Stored XSS)`/持久型`(Persistent XSS)`：网站将未正确过滤的数据储存在数据库中，例如论坛发言、用户评论等等。</X.P>
            <X.Image src="stored.jpg" width="800px" />
            <X.Oli reset={1}>攻击者发现服务器会直接保存未过滤的用户输入，因此向服务器注入一段代码，例如留下一条包含`script`标签的评论；</X.Oli>
            ...
            <X.P>DOM型`(DOM-Based XSS)`：DOM型XSS和反射型XSS很像，但DOM型XSS是由客户端的JavaScript代码处理数据时造成的漏洞，而不是服务器端。</X.P>
            ...
            <X.H2>Reflected XSS</X.H2>
            <X.P>@PortSwigger Lab: Reflected XSS into HTML context with nothing encoded[https://portswigger.net/web-security/cross-site-scripting/reflected/lab-html-context-nothing-encoded]@</X.P>
            ...
            <X.H2>Stored XSS</X.H2>
            <X.P>@PortSwigger Lab: Stored XSS into HTML context with nothing encoded[https://portswigger.net/web-security/cross-site-scripting/stored/lab-html-context-nothing-encoded]@</X.P>
            ...
            <X.H2>DOM-Based XSS</X.H2>
            <X.P>@PortSwigger Lab: DOM XSS in `document.write` sink using source `location.search`[https://portswigger.net/web-security/cross-site-scripting/dom-based/lab-document-write-sink]@</X.P>
