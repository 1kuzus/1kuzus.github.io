...
export default function Post() {
    return (
        <>
            ...
            <X.P>本文会给出所有排序算法（从小到大升序排列）的代码实现，代码可以直接提交到@洛谷 - P1177【模板】排序[https://www.luogu.com.cn/problem/P1177]@，其中的`sortArray`函数可以提交到@力扣 - 912.排序数组[https://leetcode.cn/problems/sort-an-array/description/]@。</X.P>
            ...
            <X.H2>优化</X.H2>
            <X.P>如果一次冒泡过程中没有发生交换，说明数组已经有序，可以提前结束排序。这样最好情况下时间复杂度是$O(n)$。</X.P>
            <X.H1>选择排序</X.H1>
            ...
            <X.H2>优化</X.H2>
            <X.P>可以使用二分查找优化插入排序的查找过程，不过此时平均时间复杂度仍然是$O(n^2)$，因为平均情况下仍然会涉及到元素的移动。此优化只是常数优化。</X.P>
            <X.H1>希尔排序</X.H1>
            ...
            <X.H1>归并排序</X.H1>
            <X.P>归并排序是一种分治算法，将数组分为两部分，分别排序；两个有序数组可以在$O(n)$时间内合并为一个有序数组。</X.P>
            <X.CodeBlock
                language="cpp"
                code={`
                #include <iostream>
            ...
            <X.H1>快速排序</X.H1>
            <X.P>快速排序也是一种分治算法，每次选择一个基准元素，将小于基准的元素放在基准的左边，大于基准的元素放在基准的右边。</X.P>
            <X.CodeBlock
                language="cpp"
                code={`
                #include <iostream>
            ...
            <X.P>洛谷有两组数据极差较大，因此有两个测试点MLE；力扣数据范围小因此可以通过。计数排序时间效率是非常快的。</X.P>
            <X.H2>关于空间复杂度</X.H2>
            <X.P>在对整数排序的例子中，显然只需要$O(k)$的额外复杂度；不过考虑对结构体数组按键进行排序，此时就不能简单的用一个整数表示“该键出现了多少次”，需要额外的$O(n)$空间存储其他信息，比如结构体的地址，此时整体空间复杂度是$O(n+k)$的。</X.P>
            <X.H1>桶排序</X.H1>
            <X.P>桶排序是计数排序的推广，将元素分到不同的桶中，然后对每个桶进行排序（这里可以用插入、快排、归并等等），最后依次输出。</X.P>
            ...
            <X.P>注：可能因为STL容器常数较大，在洛谷只是初始化`buckets`就花费了较长时间（前两个元素范围较大的测试点约`100ms`）。</X.P>
            <X.H1>基数排序</X.H1>
