    ...
    return (
        <>
            ...
            <X.CodeBlock
                ...
                `}
            />
            <X.P>虽然这种方法不能精确控制延迟多少秒/毫秒，但可以通过调节外层循环的次数（内层也可以）来相对地控制延迟时间。（一层循环最大执行次数是`ffffh`，仍然太快，因此需要两层循环）</X.P>
            <X.H2>随机数</X.H2>
            ...
            <X.CodeBlock
                ...
                `}
            />
            <X.P>这里生成的是`8`位的随机数，每次调用`getrand`都会更新`rand`标号处的一个字节的值。这个随机数生成方法有时会陷入`0`循环，因此产生新的随机数时加了一个判断，如果结果为`0`则重新从`seed`开始生成。</X.P>
            <X.H2>处理键盘输入</X.H2>
            ...
            <X.CodeBlock
                ...
                `}
            />
            <X.P>在设计键盘响应时，不希望玩家能够直接修改蛇的移动方向为当前方向的反向（不能调转$180^\circ$），例如如果当前移动方向为向上，则应只允许修改为向左/向右，而不能修改为向下。为此额外添加了`dirbuf`变量，用户的键盘输入会实时存入`dirbuf`，但只在每次主循环中有条件地与`dir`同步。主循环如下（其中`move`函数处理蛇移动一步，后面会详细说明）：</X.P>
            <X.CodeBlock
                language="asm8086"
                code={`
                main_loop:
                ...
                `}
            />
            <X.P>代码中`dir`与`dirbuf`异或为`3`时，表示方向刚好相差$180^\circ$，此时不改变`dir`的值（`je keep_dir`）；由于异或操作改变了`AL`的值，因此送入`dir`前需要再异或一次原来的`dir`值。</X.P>
            <X.H2>坐标设计与操作显存绘制</X.H2>
            <X.P>通过操作显存绘制图形，显存中用`2`个字节表示一个字符，低字节表示字符的ASCII码，高字节表示字符的颜色。这里不设置字符，只设置背景色，相邻的两列刚好凑成一个正方形。</X.P>
            <X.P>游戏的地图大小设计为$16 \times 16$（含边界），这样刚好可以用`4`位表示一个坐标分量，一个字节表示一个二维坐标。约定一个字节的高`4`位表示$y$坐标，低`4`位表示$x$坐标。例如`4ah`表示逻辑坐标$(10,4)$，对应$16 \times 16$地图中的第`10`行、第`4`列的方格。</X.P>
            <X.P>由于字符显示区是`25`行、`80`列，这里的$x$坐标对应了`行号-5`（为了让地图显示在屏幕中间），$y$坐标对应了`列号/2`（因为每个字符占`2`列）。封装一个`write`函数，用于将属性写入显存的指定位置：</X.P>
            <X.CodeBlock
                language="asm8086"
                code={`
                ...
                `}
            />
            <X.P>显存的起始地址是`b8000h`，屏幕第`i`行、第`j`列的偏移地址是`80\*2\*i+2\*j`，用`段地址:偏移地址`表示为`b800:80\*2\*i+2\*j`；我们希望从第`5`行开始显示地图，因此偏移地址额外加上`80\*2\*5`，如果加到段地址上还要除以`16`，也就是`80\*2\*5/16=50`，对应代码中`0b800h+50`。</X.P>
