...
const pathname = '/23c/pattern-recognition-2/';
        ...
        <>
            ...
            <X.HighlightBlock bgcolor="gray">
                ...
                <X.Uli>
                    ...
                    <X.P>
                        再看其他的三个量，{`$P(\\mu|\\bm{\\chi})$`}、{`$f(\\mu)=P(\\bm{\\chi}|\\mu)$`}、$P(\mu)$，---
                        这是贝叶斯学派的经典思想：用样本修正先验概率，得到后验概率。这三个式子都可以写为仅含$\mu$的函数。当然，因为假设的先验分布简单（均匀分布），$P(\mu)$为常数。\n
                ...
                <X.Uli>
                    <X.P>
                        ...
                        进一步地，在损失函数为平方误差时，有结论可以给出估计值{`$\\hat{\\mu}$`}
                        的具体数值，也就是`步骤4`。
                    </X.P>
                </X.Uli>
            </X.HighlightBlock>
