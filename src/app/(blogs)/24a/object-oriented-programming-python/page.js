...
const pathname = '/24a/object-oriented-programming-python/';
...
export default function Blog() {
    return (
        <>
            ...
            />
            <X.P>
                定义类时，用`class 子类(父类):`表示继承。---
                如果子类有自己的构造函数，会覆盖父类的构造函数；否则会继承父类的构造函数。
            </X.P>
            ...
            <X.Image src="mro.jpg" width="300px" invertInDarkTheme />
            ...
            <X.H1>封装</X.H1>
            <X.P>
                在前面`Unit`类的例子中，我们可以通过直接访问`u1.hp`修改其值，这样并不安全（这岂不是像外挂一样）！*封装*的目的是为了保护数据，不让外部直接访问和修改。---
                在Python中，约定通过在属性名称前加两个下划线`__`来将属性私有化。这种命名约定会使Python解释器修改变量名为`_类名__属性名`的形式，使其在类外部变得难以访问。
            </X.P>
            <X.H2>将实例属性私有化</X.H2>
            <X.CodeBlock
                language="python"
            ...
            <X.HighlightBlock>
                <X.H3>注意</X.H3>
                <X.P>
                    C++中的`protected`关键字是一种严格的访问控制机制，而Python中的单下划线变量只是一种命名约定，不具有强制性，实际上是可以访问的。---
                    只不过有时违反了这样的约定时，有些代码编辑器会给出警告。
                </X.P>
            ...
            <X.H2>举个例子</X.H2>
            <X.P>
                多态是当一个类继承自另一个类并重写了其方法时，可以在不改变原有接口的情况下，根据对象的实际类型来调用不同的方法实现。---
                这听起来有些复杂，我们来举一个具体的例子：
            </X.P>
            <X.CodeBlock
                language="python"
                code={`
            ...
            />
            <X.P>
                这个例子中，`show_info`函数接受一个`Unit`类型的参数，但是我们传入的是其子类`GroundUnit`和`AirUnit`类型的实例。---
                子类重写了父类的`info`方法，相当于共用了父类的接口，但是子类又通过继承重写了接口，从而实现了不同的功能。
            </X.P>
            <X.H2>通过抽象类实现多态</X.H2>
            <X.P>
            ...
            <X.P>
                在计算机科学中*反射*`(reflection)`是指计算机程序在运行时可以检查、访问、和修改它本身状态或行为的一种能力。---
                表现在Python面向对象编程中有四个内置函数：`getattr()`、`setattr()`、`hasattr()`和`delattr()`，可以通过字符串的形式操作对象的属性和方法。
            </X.P>
            <X.CodeBlock
                language="python"
                code={`
