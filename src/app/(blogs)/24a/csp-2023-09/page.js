...
const pathname = '/24a/csp-2023-09/';
...
export default function Blog() {
    return (
        <>
            ...
            <X.H1>T4 阴阳龙</X.H1>
            <X.P>
                看到员工数量级是比较小的，想到去维护行、列、正反对角线四个方向上的员工位置，利用C++ STL的`map`存储，---
                键值对为`(pos_val,id)`，其中这个`pos_val`在竖直方向是员工的`y`坐标，其他方向是`x`坐标（其实两条对角线方向上取哪个都行），`id`是员工的编号。
            </X.P>
            <X.P>
                `map`提供了`lower_bound`和`upper_bound`函数，可以二分找到第一个键值*大于等于*和*大于*给定键的元素；如果想找到第一个键值*小于*给定键的元素，---
                可以在判断`lower_bound`返回的不指向容器首部后，再移到前一个位置（`it--`）。
            </X.P>
            <X.P>
                在每个方向上我们都需要若干个这样的`map`，具体地说，我们需要`n`个存储垂直方向的、`m`个存储水平方向的、`2`*`n+m-1`个存储两条对角线方向的。---
                提前开好的话占空间比较大，考虑用`unordered_map`存储这些`map`。以对角线方向为例，可以用`x+y`和`y-x`作为定位到具体`map`的键，---
                我们并不需要这些键是有序的，所以用`unordered_map`就可以了。
            </X.P>
            <X.CodeBlock
                language="cpp"
                code={`
