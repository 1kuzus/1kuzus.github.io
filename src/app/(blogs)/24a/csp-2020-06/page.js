...
const pathname = '/24a/csp-2020-06/';
...
export default function Blog() {
    return (
        <>
            ...
            <X.P>
                对于这道题，$S_0=[1,0,0,0]$，它和$G^t$相乘实际上得到的是$G^t$的第一行。这也是题目中查询字符串长度为`1`时的解。---
                对于$G^t$的求解，由于$t \lt 10^9$，需要使用快速幂求解。
            </X.P>
            <X.H2>两位状态转移</X.H2>
            ...
            <X.P>
                这个图中描述的生成关系是一定由*两位都参与*了的，举个例子，`42`的下一状态是`164`，但我们并不认为`16`可以被`42`生成，---
                因为事实上`2`并没有参与这个过程（好好理解一下）。
            </X.P>
            <X.P>
                从图中还可以看出，一定不会有`12`、`24`、`22`、`11`、`21`、`14`这六个组合出现，因为他们无法被`4`或`6`生成。
            </X.P>
            <X.P>注意图中有两个环，在被`6`生成的数中有一个长度为`6`的环，在被`4`生成的数中有一个长度为`2`的环。</X.P>
            <X.H2>矩阵等比数列求和</X.H2>
            <X.P>
                这一步应该是我和绝大部分题解做的不一样的地方，大多数题解都是将`1`位或`2`位的状态合在一个大矩阵里考虑的。---
                我自己做的时候就想都往最初的`4`*`4`矩阵上靠，所以后面扯到了一些比较难搞的数学操作。
            </X.P>
            <X.P>
                现在这样考虑，举个例子，看上面的状态图，我们知道一个`6`在`t=2`时间就会生成一个`41`，不过由于有环的存在，在`t=8`、`t=14`等等也会---
                生成一个`41`。所以反过来想，如果想知道`t`时刻生成的`41`的数量，就等于：\n`t-2`时刻的`4`的数量$+$`t-8`时刻的`4`的数量$+$`t-14`时刻的`4`的数量...
            </X.P>
            <X.P>也就是需要求：{`$S_0(G^{t-2}+G^{t-8}+G^{t-14} \\dots)$`}。</X.P>
            <X.P>
                这其中有两个参数，第一个是第一项的指数，这取决于查询的两位数在图中最少需要经过几步被`4`或`6`生成；---
                第二个是等比数列的公比，如果生成数是`6`，则循环周期为`6`，公比为$G^6$；---
                如果生成数是`4`并且查询的是`46`或`66`，则循环周期为`2`，公比为$G^2$。
            </X.P>
            <X.P>
                对矩阵进行等比数列求和和普通数列一样，只不过最后的除法要转换为乘以矩阵的逆；又因为公比只有两种，所以只需要提前计算
                {`$(G^2-I)^{-1}$`}和{`$(G^6-I)^{-1}$`}。
            </X.P>
            <X.CodeBlock
                language="cpp"
            ...
            <X.P>
                首先，`3`位及以上的状态不会出现环，这是因为，我们的定义要求所有位都参与生成，对于一个三位的状态`xyz`去生成下一个状态`abc`，最极端的情况是`x`左侧结合一位生成`a`，
                `z`右侧结合一位生成`c`，但`y`只能直接生成`b`。这样中间位一定会在一或两步之后变成`1`，然后不再能生成新的三位状态。
            </X.P>
            <X.P>查询长度大于`2`的字符串，我们通过`dfs`寻找其`2`位的生成串，找到一个就按照前面的方法增加答案。</X.P>
            <X.H2>代码</X.H2>
            <X.CodeBlock
                language="cpp"
                code={`
