...
const pathname = '/24a/machine-learning-exercises/';
...
export default function Blog() {
    return (
        <>
            ...
            <X.Oli>
                <X.P>
                    正样本点是$x_1=(3,3)^T$和$x_2=(4,3)^T$，负样本点是$x_3=(1,1)^T$，求解感知机模型，初始参数取$0$。
                </X.P>
            ...
            <X.Oli reset>
                ...
                <X.P noMarginBottom>答：</X.P>
                <X.Uli>
                    有监督学习：学习器通过对大量有标记的训练例进行学习，从而建立模型用于预测。缺点在于很难获得大量有标记的样本。
                </X.Uli>
                <X.Uli>无监督学习：无训练样本，仅根据测试样本在特征空间分布情况来进行标记。缺点是准确性差。</X.Uli>
                <X.Uli>
                    半监督学习：有少量训练样本，学习器以从训练样本获得的知识为基础，结合测试样本的分布情况逐步修正已有知识，并判断测试样本的类别。
                </X.Uli>
            </X.Oli>
            <X.Br />
            <X.Oli>
                ...
                <X.P>
                    答：基于相似的样本具有相同标签的假设，构建图结构，利用节点标签扩散，将已标记样本根据样本特征的相似性进行扩散，---
                    获得未标记样本的标记。过程包括：图表示、初始化标签、标签传播、学习模型、分类预测。
                </X.P>
            </X.Oli>
            <X.Br />
            <X.Oli>
                ...
                <X.P>
                    答：在拥有部分额外有标签数据时，可利用监督信息改善聚类效果，传统聚类方法通常只使用无标签的数据进行聚类。
                </X.P>
            </X.Oli>
            <X.Br />
            <X.Oli>
                ...
                <X.P>
                    答：“伪标签”是一种半监督学习中的策略，通过使用已训练模型对未标记数据进行预测，并将这些预测结果作为“伪标签”来扩充训练数据。---
            ...
            <X.Oli reset>
                ...
                <X.P>
                    答：迁移学习是指利用数据、任务或模型之间的相似性，将在旧领域学习过的模型，应用于新领域的一种学习过程。迁移学习提供了一种将---
                    预训练模型在自己数据集上进行微调的技术。核心思路是找到源域和目标域之间的差异性，并将整体工作归为两方面：---
                    一是度量两个领域的相似性，不仅定性，还能定量的给出相似程度；二是以度量为准则，通过我们所采用的学习手段增大两个领域之间的相似性，完成迁移学习。
                </X.P>
            </X.Oli>
            <X.Br />
            <X.Oli>
            ...
            <X.Oli>
                ...
                <X.P>
                    答：领域自适应旨在解决在源域上训练的模型在目标域上性能下降的问题。在领域自适应中，源域和目标域通常具有不同的分布或特性，---
                    例如，它们可能来自不同的传感器、环境或任务。
                </X.P>
                <X.P noMarginBottom>基本原理包括：</X.P>
                <X.Uli>源域训练：在源域上使用标记的数据训练一个模型。</X.Uli>
                <X.Uli>
                    领域间差异：源域和目标域存在领域间的差异，这些差异可能包括数据分布的不同、特征的变化等。由于这些差异，---
                    直接将源域上训练的模型应用到目标域上可能导致性能下降。
                </X.Uli>
                <X.Uli>
                    <X.P noMarginBottom>域适应方法：通过调整模型，使其适应目标域的特点，从而提高性能。常见方法有：</X.P>
            ...
            <X.Oli>
                ...
                <X.P noMarginBottom>
                    答：源域是模型在训练阶段接触到的领域，通常具有相对丰富的标记数据。目标域是模型在测试阶段面对的领域，通常没有或有很少的标记数据。---
                    将源域和目标域的差异性视为两个域内样本数据的概率分布的差异性进行研究，根据采用显式或隐式的距离度量分为两类：
                </X.P>
                <X.Uli>
                    显式度量：由预定义好的距离公式产生的度量，具有特定形式。例如欧氏距离、马氏距离、余弦相似度等。
                </X.Uli>
            ...
            <X.Oli>
                ...
                <X.Uli>
                    基于样本的迁移学习：核心思想是增大源域样本中，与目标域样本差异度小的比重，根据源域和目标域的相似度来学习源域样本的权重$v_i$。
                </X.Uli>
                <X.Uli>
                    基于特征的迁移学习：学习一个作用于源域和目标域上的特征变换函数$T$来最小化迁移正则化项$R$。
                </X.Uli>
            ...
            <X.Oli reset>
                ...
                <X.P>
                    答：相似度函数是用于度量两个对象之间相似程度的函数。在机器学习和数据挖掘中，相似度函数通常用于比较两个样本、文档、图像或其他数据表示的对象---
                    之间的相似性。相似度函数的设计直接影响了模型的性能，因为它定义了模型对相似性的理解。在迁移学习中，相似度函数可以用于度量源域和目标域---
                    之间的相似性，或者度量不同任务之间的相似性。基本思想是从大规模训练数据集中学习一个相似度函数，将相似度函数用于预测，---
                    通过对比查询样本与支持集每一个样本的相似度，找到得分最高的那个。
                </X.P>
            </X.Oli>
            <X.Br />
            <X.Oli>
                <X.P>请介绍相似度函数学习的流程，并解释孪生网络和三元组损失。</X.P>
                <X.P noMarginBottom>答：流程为：</X.P>
                <X.Uli>在大规模数据集上训练一个孪生网络</X.Uli>
                <X.Uli>给定一个`n-way`，`k-shot`的支持集</X.Uli>
                <X.Uli>给定查询样本，预测其属于的分类</X.Uli>
                <X.P withMarginTop>
                    孪生网络是一种特殊的神经网络结构，有两个相同的子网络，它们共享相同的权重。这两个子网络分别处理输入样本，然后通过一个比较层将两个子网络的输出进行比较。---
                    孪生网络的训练目标是使得相似的样本在比较层中的距离较小，而不相似的样本的距离较大。对比损失是孪生网络中常用的损失函数。
                </X.P>
                <X.P>
                    三元组损失是一种用于训练相似度学习模型的损失函数，通常与三元组网络一起使用。在一个三元组中，包含一个锚点样本、一个正样本和一个负样本。---
